...
**Shared machine, concurrent writers.** A second Claude session works this same repo and branch (`check-equivariance-necessity`) from another machine and pushes frequently — **always `git pull --rebase` before committing**, and never rewrite files another session owns. ⚠️ **Queued Slurm jobs bind to commits**: exp_11 training legs verify their `EXPECT_SHA` at start via the *content-scoped* gate in `fa_orbit_train.sbatch` (Codex-approved, `da7ee7f`) — record/worklog commits are safe while legs pend, but commits touching the training closure (`train.py`, `defaults.ini`, `src/`, `data/AR`, arm configs, the launcher + its helpers) still abort them fail-closed. Never revert that gate to HEAD-identity. On neuronic, sbatch has **no `SBATCH_EXCLUDE` env var** — node exclusion works only as an explicit `--exclude`/`EXCLUDE=` argument (screen submitter only; the training submitter has no exclusion path). ECC-flaky nodes (Aug 2026): neu301/303/305/306/317/319/322/332. On the A6000 box, Yixun also runs experiments from *sibling checkouts* (`~/codespace/exp-12-arms`, `exp-08-cylvit-pe-cnn`, `exp-09-cyl-dinov3-no-ssl`, `exp-10-cyl-distill`, `exp-17-yawaug-a6000` — the last one produced the completed exp_17 FULL training this repo's C4 grid evaluates) whose jobs share those GPUs and use their own experiment numbering. **Before assuming any `train.py` process belongs to this worktree, check `readlink /proc/<pid>/cwd`; never kill or edit a run you did not launch.**

## Checkpoint storage (Yixun mandate, 2026-08-20)

**All training checkpoints live on the NAS: `/media/diskstation/yixunhu/FLAC/checkpoints/<experiment>/[<arm>/]`** — name the experiment folder descriptively (e.g. `exp19_haa_finetune/P1/`). Local `outputs_FLAC/` keeps only the checkpoints active work needs (published-row endpoints, in-flight training); archive the rest to the NAS with verification (count + per-file bytes + sampled sha256) BEFORE deleting local copies. ⚠️ NAS is CIFS: rsync needs `--inplace` (the server EPERMs rsync's mkstemp temp files — a plain rsync reports success with ZERO bytes landing); `mv` within the share is a cheap rename. Eval can read checkpoints from the NAS via symlink farms (`eval_FLAC.py` writes metric JSONs to `dirname(--ckpt-path)` — farm in your own tree so writes land locally).
...
**⚠️ Eval-protocol flags are part of the experiment, not defaults.** `eval_FLAC.py` takes `--cond-method {vanilla,fa_invariant}` (plus `--frame-avg-angles`, `--rotate-deg`/`--rotate-mode` for C₄ sweeps and the 45° negative control, `--cond-autocast {default,bf16,off}`). **`--cond-autocast bf16` is part of the registered table protocol** (`model_comparison.md` line 4) and the CLI default is `default`, NOT bf16 — omitting it makes numbers incomparable to every existing row (caught by Codex on exp_17's 128-cell grid before launch). `eval_FLAC.py` also **writes its metric JSON to `dirname(--ckpt-path)`** — never point it at another checkout's checkpoints in place; symlink them into your own outputs tree first (pattern: `exp_17…/yaw_aug_a6000_roteval_run.sh`). **The flag must match how the checkpoint was trained.** A mismatch produces plausible-looking but catastrophically wrong numbers in both directions — the fa-trained B-F@40k reads `8.202/0.978/38.79/R5.39` under fa eval and `10.652/2.082/80.86/R0.68` under vanilla eval. Evaluating fa checkpoints with the default (vanilla) conditioning caused exp_09's protocol error and one retracted exp_07 conclusion. **Put the eval-protocol flags in every launch/screen manifest; never rely on the default.**
