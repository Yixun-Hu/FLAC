...
4. `<exp name>_code_review.md` — the Reviewer's verdict on the new code (record "N/A — no code written" for code-free experiments). Name is reviewer-neutral to match the reciprocity rule; experiments predating this rule keep their legacy `<exp name>_codex_code_review.md` filename.
