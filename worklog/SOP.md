...
5. `<exp name>_<reviewer>_code_review.md` — the Reviewer's verdict on the new code, where `<reviewer>` names the reviewing model: `codex` when Codex reviews (Claude-driven session), `opus` when Claude Opus reviews (Codex-driven session) — e.g. `<exp name>_codex_code_review.md` or `<exp name>_opus_code_review.md`. Record "N/A — no code written" for code-free experiments. The file MUST also open with a header giving the reviewer's exact identity: model/product name and version, invocation method, and review date — e.g. `**Reviewer:** OpenAI Codex (codex-cli 0.142.5, \`codex exec\`, read-only sandbox) · **Date:** 2026-07-04`. **Reviews (plan and code) always use the Reviewer family's strongest available model at its highest reasoning setting** — currently `gpt-5.5` at Extra High (`xhigh`) reasoning effort for Codex, Claude Opus 4.8 at max effort for Claude.
