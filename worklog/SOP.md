...
4. `<exp name>_code_review.md` — the Reviewer's verdict on the new code (record "N/A — no code written" for code-free experiments). The file MUST open with a header identifying the reviewer: model/product name and version, invocation method, and review date — e.g. `**Reviewer:** OpenAI Codex (codex-cli 0.142.5, \`codex exec\`, read-only sandbox) · **Date:** 2026-07-04` or `**Reviewer:** Claude Opus 4.8 (max effort, \`claude\` CLI) · **Date:** ...`. Filename is reviewer-neutral to match the reciprocity rule; experiments predating this rule keep their legacy `<exp name>_codex_code_review.md` filename.
