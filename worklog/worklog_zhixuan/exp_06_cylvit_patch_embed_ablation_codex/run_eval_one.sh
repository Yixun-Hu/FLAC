...
Useful environment overrides:
  ...
  DATASET_CONFIG=/path.json  exact evaluation split override
  PROTOCOL_TAG=subset544   suffix embedded in the output name
...
case "$VARIANT" in
  ...
  cnn)
    ...
    DEFAULT_SAVE_DIR="outputs_FLAC/exp06_cylvit_pe_cnn_patchlocal_trainS${TRAIN_SEED}"
    ;;
  *)
...
case "$K" in
  1) DEFAULT_DATASET_CONFIG="src/configs/dataset_configs/AR/eval/acousticroom_unseeneval_1.json" ;;
  8) DEFAULT_DATASET_CONFIG="src/configs/dataset_configs/AR/eval/acousticroom_unseeneval.json" ;;
  *) echo "K must be 1 or 8, got '$K'." >&2; exit 2 ;;
esac
DATASET_CONFIG="${DATASET_CONFIG:-$DEFAULT_DATASET_CONFIG}"
PROTOCOL_TAG="${PROTOCOL_TAG:-}"
if [[ -n "$PROTOCOL_TAG" && ! "$PROTOCOL_TAG" =~ ^[A-Za-z0-9][A-Za-z0-9_-]*$ ]]; then
  echo "PROTOCOL_TAG must contain only letters, digits, underscores, and hyphens." >&2
  exit 2
fi
PROTOCOL_SUFFIX="${PROTOCOL_TAG:+_${PROTOCOL_TAG}}"
...
EVAL_NAME="exp06_cylvit_pe_${VARIANT}_trainS${TRAIN_SEED}_step${TRAIN_STEP}_K${K}_evalS${EVAL_SEED}_yaw${YAW_TAG}${PROTOCOL_SUFFIX}"
