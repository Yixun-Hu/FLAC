...
- `cnn`: a four-layer overlapping cylindrical convolutional stem with widths
  `3 -> 34 -> 44 -> 96 -> 512`, total stride `(16, 32)`, pointwise channel
  LayerNorm and GELU after the first three layers (791,050 parameters).

The CNN uses circular padding only on the azimuth axis and zero padding on the
elevation axis. Its total azimuth stride is 32 pixels, so the output token grid
remains exactly equivariant to the 16 panorama-aligned yaw rotations of C16.
The two patch embeddings differ by only 10 parameters.
...
`outputs_FLAC/exp06_cylvit_pe_cnn_trainS42/`; the 30k checkpoints are named
`step=000030000.ckpt`.
