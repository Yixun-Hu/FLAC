...
def expected_conditions(
    ...
    if suite == "k1":
        clean_k1 = {
            Condition(variant, train_seed, train_step, 1, eval_seed, 0.0)
            for variant in VARIANTS
            for eval_seed in TABLE1_SEEDS
        }
        return clean_k1 | c16_conditions(train_seed, train_step, c16_eval_seed)
...
def build_markdown(
    ...
    c16_only = suite == "c16"
    active_k_text = "1" if suite in ("k1", "c16") else "1,8"
    seed_note = (
        f"This C16 diagnostic uses one generation seed ({c16_eval_seed}) for the single "
        "train-seed-42 checkpoint."
        if c16_only
        else "Five clean-eval seeds are generation seeds for one trained checkpoint; they do "
        "not measure variance across independently trained models."
    )
    lines = [
        ...
        "",
        f"Protocol: train seed {train_seed}, train step {train_step}; deterministic "
        f"{sample_count}-item unseen "
        "AcousticRooms split; cfg scale 1.0; one diffusion step; vanilla conditioning. "
        f"Clean results use K in {{{active_k_text}}}, yaw 0, "
        "and generation seeds 42–46. "
    ...
    table_ks = (1,) if suite in ("k1", "c16") else (1, 8)
    for variant in VARIANTS:
        for k in table_ks:
            cells = [
                fmt_mean_std(
    ...
    lines += [
        ...
        "",
        "Differences are computed from each variant's available-seed mean. Negative is "
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--suite", choices=("all", "k1", "table1", "c16"), default="all")
    parser.add_argument("--train-seed", type=int, default=42)
    parser.add_argument("--train-step", type=int, default=30000)
    parser.add_argument("--c16-eval-seed", type=int, default=42)
    parser.add_argument("--sample-count", type=int, default=6337)
    parser.add_argument(
        "--linear-dir",
        type=Path,
    ...
    parser.add_argument(
        "--cnn-dir",
        type=Path,
        default=root / "outputs_FLAC/exp06_cylvit_pe_cnn_patchlocal_trainS42",
    )
