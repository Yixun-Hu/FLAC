...
# SUITE=k1 runs the same protocol without K=8: 40 unique conditions.
set -euo pipefail
...
Suite selection:
  ...
  SUITE=k1       K=1 clean seeds 42..46 plus C16 gate (40 unique jobs)
...
Common overrides:
  ...
  SEQUENTIAL=1              run linear then cnn; permits one shared GPU safely
...
Per-variant checkpoint/config overrides:
  ...
  DATASET_CONFIG=... PROTOCOL_TAG=subset544 SUMMARY_SAMPLE_COUNT=544
...
case "$SUITE" in
  all|k1|table1|c16) ;;
  *) echo "SUITE must be all, k1, table1, or c16; got '$SUITE'." >&2; exit 2 ;;
esac
...
SUMMARY_SAMPLE_COUNT="${SUMMARY_SAMPLE_COUNT:-6337}"

LINEAR_SAVE_DIR="${LINEAR_SAVE_DIR:-outputs_FLAC/exp06_cylvit_pe_linear_trainS${TRAIN_SEED}}"
CNN_SAVE_DIR="${CNN_SAVE_DIR:-outputs_FLAC/exp06_cylvit_pe_cnn_patchlocal_trainS${TRAIN_SEED}}"
...
if [[ "$SEQUENTIAL" != "0" && "$SEQUENTIAL" != "1" ]]; then
  echo "SEQUENTIAL must be 0 or 1; got '$SEQUENTIAL'." >&2
  exit 2
fi
if [[ "$GPU_LINEAR" == "$GPU_CNN" && "$SEQUENTIAL" != "1" && "${ALLOW_SHARED_GPU:-0}" != "1" ]]; then
  echo "GPU_LINEAR and GPU_CNN are both $GPU_LINEAR; use SEQUENTIAL=1 or distinct devices." >&2
  exit 2
fi
...
run_variant_suite() {
  ...
  if [[ "$SUITE" == "all" || "$SUITE" == "k1" || "$SUITE" == "table1" ]]; then
    for k in "${TABLE1_KS[@]}"; do
...
if [[ "$SEQUENTIAL" == "1" ]]; then
  echo "[exp06-suite] launching two sequential workers"
else
...
if [[ "$SEQUENTIAL" == "1" ]]; then
  ...
  if [[ "$STATUS_LINEAR" == "0" ]]; then
    ...
    STATUS_CNN=125
    echo "[exp06-suite] cnn skipped because the sequential linear worker failed." >"$CNN_LOG"
  fi
else
  run_variant_suite linear "$GPU_LINEAR" "$LINEAR_SAVE_DIR" "$LINEAR_MODEL_CONFIG" \
    "$LINEAR_CKPT" "$LINEAR_BARE_CKPT" >"$LINEAR_LOG" 2>&1 &
...
if [[ "$RUN_SUMMARY" == "1" && "$DRY_RUN" != "1" && "$STATUS_LINEAR" == "0" && "$STATUS_CNN" == "0" ]]; then
  ...
  SUMMARY_SUFFIX="${PROTOCOL_TAG:+_${PROTOCOL_TAG}}"
  SUMMARY_OUTPUT="${SUMMARY_OUTPUT:-$SCRIPT_DIR/eval_summary_trainS${TRAIN_SEED}_step${TRAIN_STEP}${SUMMARY_SUFFIX}.md}"
  ...
  SUMMARY_CMD=(
    ...
    --sample-count "$SUMMARY_SAMPLE_COUNT"
    --require-complete
  )
  set +e
  "${SUMMARY_CMD[@]}"
