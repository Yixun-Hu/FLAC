#!/usr/bin/env bash
# Train one ViT conditioner from the exported 5k weights to equivalent total 30k.
set -euo pipefail
...
export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/matplotlib-exp05-phase3-30k-${MODEL}}"
...
NAME="FLAC_exp05_${MODEL}_phase3_total30k_s${SEED}"
if [ "$RESUME_FROM" = "auto" ]; then
  if [ -f "$SAVE_DIR/last.ckpt" ]; then
    RESUME_FROM="$SAVE_DIR/last.ckpt"
  else
    RESUME_FROM=""
  fi
fi
...
echo "=== exp05 Phase 3: 5k -> total 30k ==="
...
if [ "$RUN_EVAL" = "1" ]; then
  MODEL="$MODEL" GPU="$GPU" SEED="$SEED" PYTHON="$PYTHON" \
    bash worklog/worklog_zhixuan/exp_05_cylvit_yaw_ablation_claude/run_phase3_milestone_eval.sh
fi
