...
## 2026-07-06T01:17:17-04:00 — W0 verdict (final, n=5 both K): FAIL — data-pipeline drift PROVEN gradient-free; exp_04 CLOSED
- **Result (W0, lr=0, 5 seeds, full split):** K=1 T60 10.132±0.054 (2.4σ), C50 1.050 (0.4σ PASS), EDT 41.10±0.10 (3.0σ); K=8 T60 8.823±0.007 (15.4σ), C50 0.966 (0.6σ PASS), EDT 38.39±0.01 (19.0σ). R@1 passes both K.
- **Analysis** — registered prediction (W0 passes) FALSIFIED, and the falsification is the finding: at lr=0 only the RIR-encoder BatchNorm running stats can move; they moved enough to cost ~30% of the full fine-tune T60/EDT damage. BN buffers drift only when input statistics differ from the stored stats ⇒ the conditioning-data statistics produced by this repo's loader differ from those the released checkpoint was trained on. **Data-pipeline drift is proven without a single gradient step.** Attribution ledger: ~30% BN/data-stats (W0), ~70% gradient adaptation toward the drifted optimum (R1b≡W1), Adam transient dead (W1), EMA ≤15% (exp_03), batch noise partial (R1b), lr excluded.
- **Decision** — registered stop executed: W2–W4b not run. exp_04 closes with results/analysis; exp_05 decision package prepared for Yixun (option A: drift bisection starting at reference-RIR preprocessing — max_len/padding/normalization in AR_md.py vs release lineage; B: matched-comparison route; C: from-scratch fa_invariant training).
