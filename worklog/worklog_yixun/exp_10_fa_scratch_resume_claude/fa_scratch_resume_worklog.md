...
- **Measured (train mode, bf16, C4, 3 seeds/rung, real DINOv3 + real AR samples):** batched-vs-loop `rel_norm` — micro-8 (3/3 angles share a draw): context_poses_vit 4.424e-02, source_vit 3.285e-02; micro-32 (2/3 share): 4.280e-02, 3.529e-02. **Ratios 1.03x and 0.93x — i.e. no rung dependence; the pre-registered prediction is REFUTED.** Sharing 2 of 3 angles already correlates essentially as much noise as sharing 3 of 3.
...
- **Revised leading explanation for exp_11's reversal:** not "exp_11's small rung amplifies sharing", but simply "**their FA trained with chunk-shared draws; ours (July code) trained with per-angle draws**". Circumstantial support: under the exp_11 recipe their *vanilla* IMPROVED vs ours (T60 8.048 vs 8.993, EDT 37.32 vs 40.62 at 40k) while their *FA* DEGRADED vs ours (EDT 41.50 vs 38.81) — the change that can only touch FA moved only the FA arm.
- **Actionable knob discovered:** `FRAME_AVG_MAX_FWD_SAMPLES` (`src/data/yaw_rotation.py:45`, default 64) sets `angles_per_chunk = 64 // micro_batch`. **Setting it equal to the micro-batch restores per-angle draws (the July behaviour); leaving it at 64 gives chunk-shared draws.** Any FA arm must therefore DECLARE this constant alongside its rung — two FA runs at the same nominal recipe are not the same method if it differs.
...
- Probe: `rope_sharing_rung_probe.py` (conditioning only; reuses exp_11's reviewed helpers). Raw: `a5_rope_sharing_rung.json`. Codex review queued.
