...
## 12. AMENDMENT Rev 3 (2026-08-13, Yixun query 3): chunked chain training

**Motivation:** L40 pool saturated; the monolithic 24 h job 3687499's estimated start keeps slipping (>24 h). Short jobs backfill into small gaps.

- **Chain:** INITIAL leg trains 0→2500; RESTART leg k trains k·2500→(k+1)·2500 from the previous boundary checkpoint; **the pre-registered 40,000 endpoint is unchanged** (the 40k pin becomes the chain cap; leg boundaries must align with the 2500 checkpoint cadence). `LEG_STEPS` env-tunable (default 2500 per Yixun's example; larger legs halve per-leg startup overhead if backfill allows).
- **Self-chaining ("watchdog"):** each leg's epilogue — strictly AFTER its completion audit passes on the boundary checkpoint (round-2 rigor, step == leg target) and the registry leg entry is written — submits the next leg via the reviewed submitter (RESTART, `--resume <boundary ckpt> --expected-step <boundary>`). The intended next-leg command is recorded in the manifest BEFORE submission so recovery is mechanical. Login-node rules forbid a persistent daemon; the session Monitor supervises (stall/failure alerts, manual resubmission as fallback). `CHAIN=1` gates all new behavior; `CHAIN` unset ⇒ the reviewed monolithic behavior, byte-compatible.
- **Per-leg pins:** time 1:30:00 (2500 steps ≈ 42 min at ~1 it/s + startup + audit margin); all existing gates run per leg; completion audit's step assertion = the leg's target (40000 ⇒ also writes `final_ckpt_sha256` and ends the chain).
