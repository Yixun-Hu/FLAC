...
## 2026-08-11 — content-scoped commit binding for training legs (two-writer fix)

Restart legs 3678599–602 were dead-on-start: EXPECT_SHA recorded 0f0acb2 at submit, but the second session commits into this same checkout and HEAD moved twice before the legs could start; the old gate required HEAD identity. Fix (Codex round 1 REVISE with 5 blocking findings → round 2 **APPROVED**, artifacts `fa_orbit_codex_content_gate_review{,_r2}.md`): the start gate now accepts HEAD ≠ EXPECT_SHA iff the training closure (train.py, defaults.ini, src/, data/AR split JSONs, the five enumerated arm configs, the launcher + its four runtime helpers, exp_07 FLAC_AR_BF.json) is byte-identical between EXPECT_SHA and the captured HEAD OID. EXPECT_SHA must be the full 40-hex OID (symbolic refs refused); HEAD re-read post-diff closes the mid-gate race; drift gate now git-quoted pathspecs + data/AR + fail-closed. Registry/manifests/worklog deliberately outside the closure — record commits can no longer kill queued legs. Guard-tests: synthetic dangling-commit fixtures (identical-tree pass / train.py-changed abort / EXPECT_SHA=HEAD refused), missing fixture = FAIL. Legs 3678599–602 + VANL 3680734 cancelled and resubmitted under the new gate.

## 2026-08-12 — R2/R3 MECHANISM READOUT (29 cells landed overnight; pin 89f24cd, s42 K8 @40k)

**R3 yaw-flatness (delta vs own rot0):** in-group rotations are at the invariance floor for every arm (|ΔT60| ≤ 0.001, |ΔEDT| ≤ 0.011 — D3-floor sanity PASS). Off-group: C4L degrades steeply (rot45: T60 +1.334, EDT +3.234, R@1 −1.231); C8 max residual T60 +0.104 (rot22.5); C16/C32 flat everywhere tested (|ΔT60| ≤ 0.026). **Finer orbits deliver the designed robustness** — the R1 accuracy cost is the price, not a failure of invariance.

**R2 decomposition (Cn/an − C4L/a4 = eval + train + interaction), T60/EDT:** the θ=0 degradation is **training-side dominated** (train T60 +0.408/+1.024/+0.737 and EDT +2.497/+4.660/+3.122 for n=8/16/32 — reproducing R1's inverted-U at C16), with a lineage-independent **eval-side penalty ~+0.45 T60 / ~+1.0 EDT that saturates already at a8** (C4L: eval +0.456/+0.451/+0.453 for a8/a16/a32; legacy exp_07 B-F replicates: +0.488/+0.480/+0.480, loop-era baseline, batched cross cells — era-mixed until cell 3684155 lands) and a consistently negative interaction. Cross contract note: fa-arm sets were incomplete (only a4 off-diagonals submitted initially) — 6 completing cells queued (3684137–40, 3684143–44); C8/C16/C32 cross numbers are PROVISIONAL until their full sets validate.

**Synthesis with q9:** training WITH finer orbits hurts θ=0 accuracy (training-side), and *evaluating* any C4-trained model with finer averaging also hurts (eval-side, saturating) — while in-group invariance is exact. Combined with the VANL reversal (fa itself underwater vs vanilla in this recipe), the working hypothesis: orbit averaging in *training* is the damaging ingredient in the L40/8×8/batched recipe, candidates = chunk-shared RoPE draws + BN-8 micro-batch statistics under averaged conditioning.

**Gate saga closed:** legs died a second time on a src/tests-only commit (exp_14/15 TDD churn); ':(exclude)src/tests' added to closure + drift gates (Codex r3 APPROVED). Legs live as 3684149–53 (EXPECT_SHA at the r3-fix commit).

## 2026-08-12 — R2 FINAL: full cross matrix validated (3/3 per fa arm); legacy replication era-clean

Full cross matrix (T60 delta vs own protocol, s42 K8 @40k): C4L at a8/a16/a32 = +0.456/+0.451/+0.453 (flat in n); C8 at a4/a16/a32 = +0.107/+0.047/+0.042; C16 at a4/a8/a32 = +0.079/+0.023/+0.000; C32 at a4/a8/a16 = +0.012/+0.026/+0.004. **Asymmetry:** finer-trained arms are near-robust to ANY eval orbit change (all ≤ +0.107, shrinking with training orbit); only the C4-trained model pays a large, immediately-saturating toll under finer eval orbits. Legacy exp_07 B-F @40k vs its own batched-era a4 baseline (cell 3684155, same pin 89f24cd): a8/a16/a32 T60 +0.490/+0.481/+0.482, R@1 −0.47/−0.71/−0.62 — replicates C4L within ~0.03 across lineage AND recipe. R2 decomposition unchanged from the provisional numbers (training-side dominated, inverted-U at C16; negative interaction). Evidence: 6 completing cross cells (3684137–40, 3684143–44) + legacy baseline committed with sidecars; manifest updated.

## 2026-08-12 evening — restart-leg wandb collision fixed (round 4, Codex APPROVED)

C4L leg 3684149 passed every start gate (content binding worked in production: HEAD had moved, surfaces identical, leg started) then died at train.py:193 — the launcher resumed the INITIAL wandb run (WANDB_RESUME=must) and prefigure's push_wandb_config (no allow_val_change) refused the restart's changed config (max_steps 40000→100000; ckpt_path next in line). C8 died identically. Fix: RESTART legs mint a fresh lineage-tagged run id (exp11-<ARM>-r<step>-<ns>-<uuid8>) and do not resume; train.py untouched (exp_15 admission pins it byte-identical). Cost: wandb curves split across runs per leg (figures come from eval JSONs; step axis aligns). Codex round-4 APPROVED (fa_orbit_codex_wandb_restart_review.md); reviewer caveats logged: PL passes resume="allow" internally so only an exact id collision could resume (ns+uuid makes that negligible), and the restart admission contract still only admits the audited 40k anchor — a mid-extension crash >40k needs an add_anchor extension before it can restart from its own checkpoint (open item, not needed until first such crash).

## 2026-08-13 — ROUND 5: CHUNKED TRAINING KIT (Codex 5-round loop → APPROVED)

Yixun's commission: the partition never backfills 34–160 h 8-GPU legs (five extension legs sat PD >16 h), so each 40k→100k extension now runs as a chain of short chunk jobs (2500 steps; VANL 5000 per the overhead analysis), driven by `fa_orbit_chunk_watchdog.sh`. Kit: submitter `--chunk-end` + per-arm CHUNK wall pins (C4L 3h/C8 4h/C16 6h/C32 9h/VANL 2.5h) + per-arm submit-reservation flock; launcher CHUNK_END (STOPSTEP feeds train.py AND the completion classifier) + post-classification endpoint attestation; preflight `--chain` (full-ancestry, per-link manifest+attestation+resume-field verification, per-link sacct verdict with EXPIRED-only leniency); recorder chain links (immutable, idempotent, sacct-COMPLETED-gated, attested-path verified); watchdog (singleton flock, terminal-only settlement, CANCELLED⇒freeze, settled-job-id manifest lookup, stale-ckpt retry-from-tip with -v1 versioning, deep startup anchor validation, per-arm max-chunk table, freeze-after-2 with human-clearable state). Registry excluded from both drift gates (lineage record, not a measurement surface). Review artifacts `fa_orbit_codex_chunk_kit_review{,_r2,_r3,_r4,_r5}.md`: r1 7 blockings, r2 4, r3 2, r4 1, r5 APPROVED. B5 (eval-side verify_chain admission of chunk ckpts) DEFERRED by scope ruling to the traj round — no >40k eval may run before it lands. VANL anchored (`1095f493…`). Guard suite 149 pass / 5 dirty-tree (clear on this commit). Chunked resume is not bit-exact vs monolithic (PL restores no RNG/dataloader position; now per-chunk) — 100k rows will carry a "chunked lineage" label.

## 2026-08-30 — ALL FOUR REMAINING ARMS FROZE ON SICK NODES; EXCLUDE added to the training submitter (round 6)

Overnight state: VANL DONE at 100k (Aug 19). C4L and C8 advanced to 85k (18 links each), C16 to 67.5k (11), C32 stuck at 45k (2). Then: Aug 27 ~09:2x, C32 leg 3759513 and C8 leg 3759739 both FAILED within minutes on **neu306**; Aug 30 ~01:1x-01:4x, C4L leg 3772907 and C16 leg 3772936 both FAILED via NCCL-watchdog hang on **neu301**. All four nodes-of-death are on the ECC sick list. Each arm hit the 2-consecutive-failure freeze; watchdog exited (every arm DONE or FROZEN) — exactly as designed. Disk == recorded tips everywhere (failed legs saved nothing); lineage clean.

Root cause: the training submitter had NO node-exclusion path (screen submitter only), and ~24 node-lottery rolls per arm made sick-node hits inevitable. Fix: `EXCLUDE` env in fa_orbit_submit.sh, DEFAULT = the 8-node sick list, `${EXCLUDE-...}` semantics (explicit empty disables), shape-validated before the drift gate, `--exclude` appended to sbatch argv, `exclude` line in the intent manifest. Codex micro-review: REVISE (one guard-test false-PASS hole) → fixed (rc + DRYRUN-line required) → production delta judged sound. Freezes cleared and watchdog restarted after commit.
