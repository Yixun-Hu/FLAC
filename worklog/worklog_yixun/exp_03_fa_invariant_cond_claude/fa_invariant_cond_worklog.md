...
## 2026-07-05T06:24:00-04:00 — R1 GATE: FAIL — diagnosis in progress (EMA hypothesis)
- **Result (R1 control vs exp_01, full split, 5 seeds, exp_01 protocol):** K=1 T60 10.736±0.069 (9.7σ), C50 1.101±0.008 (5.2σ), EDT 44.59±0.12 (11.9σ); K=8 T60 9.614±0.013 (57.6σ), C50 1.032±0.002 (16.7σ), EDT 42.31±0.05 (62.4σ). All primary metrics FAIL the pre-registered 2σ gate; R@1 K=1 passes (1.8σ), K=8 fails advisory (3.7σ). Per protocol, R2 is NOT launched and R2/R3/R4 numbers are not to be read against H3 until a control passes.
- **Hypothesis (stated before the diagnostic):** the released FLAC_EMA weights are an EMA average — a smoothed point, not a critical point of the training loss. Online SGD from that point, without EMA re-averaging, drifts to "online-weights quality" REGARDLESS of lr — which would explain both this failure (clean recipe, lr 5e-6, 10k steps) and the pre-revert one (lr 5e-5, 2k steps). Prediction: the shipped non-EMA `FLAC.ckpt` evaluates ≈ our regressed control (T60 ~10.5–11 K=1 / ~9.5–9.8 K=8), NOT ≈ the 9.97/8.61 EMA baseline. Diagnostic running (K=1/K=8, seed 42; --allow-partial-load for the PL-wrapper format). Infra note: first attempt killed by the 10-min foreground cap — infrastructure, retried in background.
- **Decision rule (pre-registered now):** if the prediction holds → the registered lr-2e-6 iteration would fail the same way (endpoint is still online-quality); the ONE recipe iteration becomes **re-enable EMA, warm-anchored** (EMA initialized from the loaded weights, effectively-constant high decay ~0.999–0.9995, no fresh-EMA warmup — the pre-revert pitfall), evaluated on the EMA weights. If the prediction fails (online ckpt ≈ EMA baseline) → proceed with the registered lr 2e-6 iteration unchanged.

## 2026-07-05T07:01:45-04:00 — corrected diagnostic; AMENDED single iteration R1b (batch parity)
- **Result (diagnostics):** first online-ckpt eval was CONFOUNDED (eval loader auto-remaps EMA keys in wrapper ckpts — measured EMA twice); corrected eval on EMA-stripped weights: K=1 10.06/1.087/40.71, K=8 8.68/1.011/37.96 — within +0.1 T60/+0.8 ms EDT of the EMA baseline, i.e. EMA-vs-online explains ≤~15% of the R1 regression. Discovery (README + defaults.ini): original training ran **effective batch 128** (32×2 accum×2 GPUs); our R1 ran effective 8 — a 16× gradient-noise mismatch invisible to the config parity audit (CLI-side).
- **Decision (amendment, declared before running):** the pre-registered lr-2e-6 iteration is superseded — under the noise model it predictably still fails (reduces drift ~2.5×, needs ~20×), and mechanically running a predicted-fail iteration wastes the compute the registration was meant to protect. The ONE recipe iteration becomes **R1b: batch 4 × accumulate 32 = effective 128 (original parity), 625 optimizer steps (identical 80k-sample budget to R1), lr 5e-6 constant, all else unchanged**. Same micro-batch count as R1 → ~3 h. If R1b fails the gate → registered STOP: no R2, full analysis for Yixun.
- **Command** — R1b per command file pattern: `--batch-size 4 --accumulate-grad-batches 32 --max-steps 625 --checkpoint-every 200 --save-dir outputs_FLAC/exp03_R1b_vanilla_ft --name FLAC_exp03_R1b_vanilla`.

## 2026-07-05T11:27:40-04:00 — R1b GATE: FAIL → REGISTERED STOP (no R2/R3/R4)
- **Result (R1b, effective batch 128, 5 seeds, full split):** K=1 T60 10.467±0.070 (6.2σ), C50 1.078±0.010 (2.7σ), EDT 43.27±0.18 (8.1σ); K=8 T60 9.196±0.009 (38.6σ), C50 0.995±0.003 (6.2σ), EDT 40.51±0.05 (41.9σ). **R@1 PASSES at both K (z=0.16/0.11 — retrieval fully preserved).**
- **Analysis** — batch parity removed ~35–40% of the R1 regression but the control still fails the primary gate. Combined evidence (R1, R1b, true-online diagnostic): ANY fine-tune of the released checkpoint with this repo's training path degrades energy-decay metrics while preserving AGREE-space retrieval; the effect is not caused by fa_invariant (never enabled in either control), not by EMA-vs-online (≤15%), and only partially by batch noise (~35–40%). Leading remaining suspects for the analysis doc: fresh Adam second-moment transient (optimizer state not released → early steps take outsized parameter-relative moves at any lr), and possible drift between the released-training code and this repo's training path.
- **Decision** — pre-registered stop honored: both permitted control attempts exhausted; R2/R3/R4 not launched (H3/H2 unresolved via fine-tuning; H1 already settled at the infrastructure level). exp_03 concludes with results + analysis; follow-up options quantified in the analysis for Yixun's decision.
