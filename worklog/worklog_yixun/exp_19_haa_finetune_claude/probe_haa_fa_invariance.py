...
def _build_stack(model_config_path, dataset_config_path, device):
    """Real conditioner + one HAA batch. Imported lazily: the pure core must stay
    importable (and unit-testable) without transformers, DINOv3 or the dataset."""
...
def main(argv=None) -> int:
    ...
    try:
        conditioner, dl = _build_stack(args.model_config, args.dataset_config, device)
    ...
    def cond_fn(sample):
        ...
        return {cid: value[0].detach().float().cpu() for cid, value in out.items()}
