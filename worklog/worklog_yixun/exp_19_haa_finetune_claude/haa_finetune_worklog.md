...
## 2026-08-21T12:05 — CYL arm INVALIDATED and stopped (cross-session finding, verified)

- **Another session (CYL-SSL setup) found, and I independently verified: this branch's `src/models/conditioners.py` has NO cylindrical routing** (0 occurrences vs exp-09's 13; the `implementation`/`gauge` keys have no consumer). The CYL run built a VANILLA DINOv3: its log carries the vanilla banner once and the cylindrical banner zero times. Cylindrical-trained weights ran through the wrong RoPE spectrum, no XYZ gauge, wrong readout. **All CYL training/eval from today is meaningless.**
- **Why my gates missed it (the honest chain):** the cylindrical model adds buffers/hooks, not parameters, so the strict load succeeded; every contract I built validates what the CONFIG SAYS, none validates what the MODEL IS; and the one runtime probe that would have caught it was skipped for CYL by my own trivial-orbit ruling. My earlier claim that the backbone was "correctly built and loaded" was asserted from config+load success without banner evidence — retracted.
- **Actions:** CYL train/launcher/eval-chain killed (this session's own jobs); outputs quarantined as `outputs_FLAC/exp19_HAA_CYL.INVALID_vanilla_arch`; no CYL row enters any table. The finding file (untracked, other session's) stands as the evidence record.
- **Structural lesson for the port round:** add a RUNTIME contract gate — assert the constructed ViT class + a roll-equivariance probe on the built model — "config says" is not "model is".
