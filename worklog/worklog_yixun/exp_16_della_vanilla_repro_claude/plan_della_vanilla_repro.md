...
## Hardware amendment #2 (Yixun-approved 2026-08-20): chain moves to ailab H200

ailab group membership confirmed (`id`: mae blanchette ailab; partition AllowGroups=cses,ailab; real held-submission probe accepted). The A100 chain waited 7 days with 0/10 legs started (gpu partition 1,565 pending; account fairshare 2.7× over-share). Chain re-targeted: `--partition=ailab --qos=ailab --gres=gpu:h200:1` (141 GB, Hopper — same architecture family as the paper's H100, closer to paper-literal than A100; the gpu80 constraint is dropped as H200-native). Recipe, seeds, budget, chunking unchanged; the chain had trained zero steps, so this is a pure venue change. A100 smoke pricing stays the ETA basis (H200 ≥ A100 throughput ⇒ 4:30:00 leg limit remains safe); leg-1's measured H200 rate is recorded on arrival. Phase-3 evals stay on the gpu/A100 partition where Phase-1 anchored calibration. Disclosure joins §1's list: training arch is now H200 (vs release H100, vs exp_16-plan-Rev-3 A100).

**Phase-3 venue amendment (Yixun-approved 2026-08-21, recording per round-E review N3):** the 13 primary Phase-3 cells run on ailab H200 (gpu/A100 remains jammed; measured cross-arch eval drift is ~1e-4, three orders below the bands); one duplicate of the headline unseen-K8-seed42 cell runs on A100 (`_a100` eval-name) as the architecture spot-check, non-gating.
