...
**Status:** REV 2 — revised per fallback plan review (`della_vanilla_repro_opus_plan_review.md`, verdict REVISE, all B1–B7 addressed, N1–N14 folded in or explicitly scoped). Awaiting Yixun approval.
...
- `della_train.sbatch` — `--gres=gpu:h100:1 --cpus-per-task=10 --mem=64G` (persistent_workers ⇒ ~16 resident workers, N5), `--time` from the smoke-measured ETA ×1.5, QOS `gpu-medium` default (N7); resume-aware (`--ckpt-path` latest if present; PL resume non-bit-exact — disclosed); refuses `srun -n>1` (N13).
