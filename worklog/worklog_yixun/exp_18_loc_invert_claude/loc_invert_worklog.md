...
## 2026-08-19T21:45:20-0400 — R-1b PASSED (all acceptance criteria); results recorded
...
## 2026-08-19T22:35:00-0400 — r6 review: APPROVE-WITH-CHANGES; R0 GO; R1 = pre-declared 3,199-prefix

- **Result** — No result-corrupting defect. Confirmed twin-leak: Bathrooms_idx_16 GT∈{S4,S7} queries structurally leak (twin label in context w.p. 8/9; first at stream index 3205); the guard's abort is CORRECT and stays. **Planner rulings:** (1) R1 runs as the plan §2.5 labelled bounded dev slice — pre-declared prefix of exactly 3,199 identities (`--smoke --max-queries 3199`), smoke-stamped/quarantined by construction; τ selection from its rows via `--mode reaggregate`. A full-seen R1 would need a reviewed group-aware context sampler — NOT pursued (τ is one scalar; 3,199 queries suffice). (2) LOW fixes as r6b, disclosure option (clean-row golden + narrowed byte-identity claim in docs; survey nonzero exit on errors) — no row-schema change, so R1's rows are final-schema; r6b lands after tonight's launches, scoped away from the running driver. (3) Results wording: gt_rir canonical file = "group representative", not "same measurement" (reviewer measured only 6/25 and 8/25 twin-pair windows identical).
- **Version Control** — review saved `loc_invert_codex_code_r6_review.md`.
- **Acceptance criteria (R1-slice, pre-launch)** — identity audit clean over the 3,199-prefix (any substitution aborts); zero leakage aborts (prefix ends before 3205 by construction); rows JSONL complete with per-sample sims at full precision; publishes smoke-stamped artifacts; per-query probe timings present; runtime logged.

## 2026-08-19T22:55:00-0400 — R0 PASSED + scorer-noise measured + R1-slice LAUNCHED

- **Result (R0)** — passed all criteria: 4 smoke queries (Cafe_idx_1), K=8, real generation end-to-end; identity gate 4/4; smoke-stamped artifacts. **Probe: 1.32 s/query mean** (conditioning 0.257 / sampling 0.399 / decode 0.541 / embed 0.097 s), peak 1.57 GB ⇒ **R1 ≈ 1.2 h, R2 ≈ 2.3 h/seed** — an order of magnitude under the §9 escalation threshold; the naive 8.7–17 h bound was pessimistic (conditioner amortization + GPU decode dominate the win). Smoke-slice localization (n=4, anecdotal): FLAC median 4.47 m vs matched baseline 8.14 m.
...
## 2026-08-19T23:45:00-0400 — R1 ABORTED at 1194 by the identity gate (REAL silent substitution caught); prefix re-declared 1,194; relaunch

- **Result** — The C2 gate caught a genuine silent substitution mid-scoring: position 1194 (`Office_idx_15/S009_R092`) is a **near-silent RIR** (absmax 9.2e-4; `is_silence()`=True after pad-crop) that the release `SampleDataset` deterministically replaces with a random item — every historical seen-split eval substituted it invisibly. Probe over ALL 6,217 seen files (driver enumeration order): **exactly 1 problematic item, position 1194**; unseen split had zero substitutions across R-1b's full pass. Classification: **dataset property; the gate worked; no code bug.** No corrupted artifact published (.partial quarantined, gate refused).
- **Analysis** — R1 prefix re-declared: **1,194 queries** (positions 0..1193; proven clean by the aborted run's own verified rows; well under the 3,205 leak bound). τ selection on 1,194 queries across ~25 rooms remains ample for one scalar. Aborted log kept with `_ABORTED_silent_rir_substitution` suffix per SOP. Lessons: (i) readback's one-wav-per-(room,source) sample can miss per-receiver silent files — a full-decode readback option goes into the r7 batch with the r6 LOW fixes; (ii) watcher `pgrep -f` patterns must not match the watcher's own shell (previous completion watcher never fired — killed).
- **Acceptance criteria (R1-v2)** — identity gate clean over the 1,194-prefix; end gate passes at the declared count; publishes smoke-stamped final artifacts (no .partial residue); per-sample sims complete for the τ sweep.

## 2026-08-20T00:25:00-0400 — R1-v2 PASSED; τ=0.02 registered; R2 registration manifest committed
...
## 2026-08-20T16:20:00-0400 — R4-r1 delivered (rir_metrics, 51 tests); Planner rulings

- **Version Control** — `837ed46` (M1/M5+registerables) `08e747b` (M2/M3) `f94ee27` (M4+compute_metrics+control) `f509abd` (ledger). Suite 590 green (1 GPU test deselected while the campaign holds cuda:1 — re-run due at the next GPU gap).
- **Analysis — rulings on the 4 flags:** (1) even-K median = np.median convention — ACCEPTED (consistency with scoring.summarize). (2) M2 device-adapted window vs the CUDA-locked repo helper, formula source-pinned — ACCEPTED as a necessary, tested deviation; recorded in the future manifest. (3) M3 FFT band-masking octaves — ACCEPTED (deterministic, dependency-free); manifest states the filter definition explicitly. (4) M4 silent-candidate NaN → query-wide uniform feature drop — **KEPT as the registered rule** (it is the plan's uniform-mask semantics), plus a required per-query diagnostic (dropped-feature count + causing candidate) so the analysis can quantify whether it ever mattered — reporting, not tuning. (5) inert placeholders — correct by design.
- **Result** — R4-r1 OPEN pending its Codex review (batched with R4-r2, one consolidated review before the seen calibration runs — both rounds are pure-additive CPU code, and calibration is the first execution).

## 2026-08-20T17:00:00-0400 — R4-r2 delivered; rulings; consolidated review launched

- **Version Control** — r4m2 commits `b1373e7 37054d5 b7ecca1 a90f9dc 529ee53 d2371f4`. Suite 623 green (+1 deselected GPU test, both devices busy). Real finding from TDD: repo `_edt`/pyroomacoustics T30 RAISE on non-decaying signals → now NaN → uniform-drop (a silent candidate would have crashed a 6,337-query run).
- **Analysis — deviation rulings:** (1) seen runs emit the full Δmax grid, registered runs refuse grids — ACCEPTED (calibration necessity). (2) one `--registration-sha` for protocol+metrics — ACCEPTED: the metric-manifest commit will contain the R2 manifest byte-identical, so a single newer sha (ancestor of HEAD) verifies both; documented. (3) metrics-retrieval own stem — ACCEPTED. (4) vestigial `outcome` param — removal queued to the review round. (5) M4 features as JSON float64 (exact round-trip) vs hex — ACCEPTED, documented inconsistency. Calibration replay will ALSO dump R1-prefix waveforms (announcement 08; ~4 GB).
- **Next** — consolidated Codex review (R4-r1+r2) → seen calibration replay on the first free GPU → metric manifest freeze.

## 2026-08-20T17:50:00-0400 — R4 consolidated review: NO-GO (1B/5H/1M/1L) → r4m3 fix batch; SEED 44 HELD for dump-parity

- **Result** — Real defects: M1 residual ≠ registered formula (probe: 1.0 vs 3.9e-5 low-energy); pyroomacoustics −1 sentinel defeats NaN-on-non-decay; M4 μ/σ unappliable (registration BLOCKER); declared secondaries/sensitivities unreportable; controls split/oracle-index defects; publication-order gaps; **firewall finding: r4m2 replaced the dump route even with --metrics off** ⇒ registered seed 44 HELD until legacy parity is proven by golden test (seeds 42/43 unaffected — r7-era code resolved in memory at their 15:55 start).
- **Analysis** — Campaign impact: R2b seed 44 slips to ~21:30; registered analysis tonight → late night/morning. R4 calibration + freeze slip ~1.5 h; unseen passes still overnight. Confirmed-pass list is substantial (M2/M3/M5 conventions, composition guard, hex codec, policy-identity) — the batch is well-bounded.
- **Next** — r4m3 (findings 1–6 + 8 + the firewall golden parity of 7) → focused re-review → seed 44 + calibration in parallel → freeze → overnight unseen.

## 2026-08-20T17:05:00-0400 — r4m3 delivered (661 tests; seed-44 parity proven); rulings; re-review launched

- **Version Control** — `d4f471a` (F7 parity vs the r7 module itself, byte-identical npz) `f360b3c` (F2 literal M1 + F3 sentinel→NaN vs real wrapper) `408a093` (F4+constants) `81dda12` (F1 authoritative manifest + F8) `a4102a5` (F5+F6) `0691d2d` (ledger).
- **Analysis — rulings:** (1) calibration identities = the R1-v2 PUBLISHED rows (1,194; the 3,199 figure was the pre-abort declaration) — passed via --calibration-identities at calibration time. (2) one-sha/two-manifests as implemented — ACCEPTED. (3) matched-control PRIMARY numbers computed offline from the replay rows' per-family ctx distances (analysis-time; zero stream doubt); metrics-retrieval supplies oracle/eligibility — ACCEPTED. (4) sensitivities opt-in, stride registered — ACCEPTED (calibration passes the flag). (5) z-ddof=0 population std registered — ACCEPTED.
- **Result** — Seed 44 remains held only on GPU availability now (parity commit green); focused re-review is the calibration gate.

## 2026-08-20T17:55:00-0400 — r4m4 verified; calibration GO under reviewer's conditions (Planner closure)

- **Version Control** — `73d5e1d` (F6 publish-as-set) `956fb6c` (F1 source_sha + r2-digests + fail-closed calibration auth) `9ba2caf` (F4 M4 battery + F5 mandatory binding + nits) `7434558` (battery coverage pin) + ledgers. Planner re-ran the two touched suites: 441 green. The r4m3 reviewer's calibration GO-conditions (publication order, M4 battery/claim honesty, fail-closed authentication) are each satisfied and test-pinned; round closed under SOP's Planner-verification clause. Flags 1–4 accepted as ruled (rir_metrics-only source pin; byte-equality without ancestry; absent-battery reported; stride reported).
