...
| `eed430f` | r5 (Amendment 4): Mapping A derives its OWN scalar over its own union (registered formula; clamp binds at 2.0401 -> x2.0, asserted pre-write in canonical mode), `--scalar` becomes an assertion, H byte/content reuse REMOVED (overlap + both scalars recorded instead), registered cross-mapping scale disclosure in marker/pointer/splits/audit/contrast report, identity `amplitude_scalar` 2.0; union resampled once per run |
| `d077ea0` | measurement (read-only): `mappingA_amplitude_window.json` — 21 sub-threshold captures ALL context-only (19 items, FurnishedRoom p008); 6 clip at x3 (2 EmptyRoom + 4 Furnished, one a target); no global window (8.7414 > 2.0401); targets-only window [0.545, 2.040] with x2.0 at −48.7 dBFS / 11.3 dB margin |
| `eaed890` | r5 (Amendment 4.1): role-aware silence gate (fatal for TARGET-role only; context sub-threshold RECORDED with affected item ids), clip gate unchanged over all files, per-item + aggregate near-silent-reference disclosure, `contrast_with_sensitivity` with/without-p008 labelled rows (primary keeps p008) |
| `a3d629b` | r5: canonical `audio_union_sha256` PINNED to `b19eff06…` from the clean dry run (generation `5fc096147bec`); `unpinned_identity_keys` now empty for both Mapping-A kinds; shape check → exact-value check + wrong-value negative, placeholder mechanism kept under its own test |
| `74ff6f4` | r5 (Amendment 4.2): listener-map scale MINIMUM becomes a recorded diagnostic (`min_side_gates`, default strict; max side still fatal in both modes); per-map/per-item near-field disclosure into the depth QA record + marker; `mappingA_stats` exclusion moves to ITEM grain with `FLAGGED_ITEMS` (19 near-silent + 4 near-field = 23) driving one labelled "minus-flagged" sensitivity row |
| `77c687c` | r5 (Amendment 4.3): `LISTENER_MAX_MISS_RATE = 0.007` registered for listener maps and resolved BY MODE (`--max-miss-rate` default None; cap resolved before the identity is built); source maps unchanged at 0.25% so the published Mapping-H marker stays valid; abort message names the mode's cap; boundary tests at 700/701 of 100,000 and the sweep's measured worst 860/131,072 |
| `8672ac7` | measurement (read-only): `mappingA_furnished_qa_failures.json` — all 20 FurnishedRoom QA failures reproduced exactly over 144 slots; TWO classes: 17 = the 4.3 audit-clamp wiring bug (miss rates 0.251–0.656%, every passing slot ≤0.243%), 3 = normal-sign containment artifact (0.30–0.83 m clearance, odd crossings on all six axes, vertical gate passes on all 20) |
| `0999bf9` | r5 (4.3 wiring fix): the canonical anti-forgery clamp follows `registered_miss_cap(listener_mode)`; mode comes from the caller, never from the audited report; listener 0.5% clean / 0.8% fails / source 0.5% fails |
| `d4aef6c` | r5 (Amendment 4.4): containment = majority-of-6-axis ray-crossing parity (≥4/6 odd ⇒ inside), FATAL in both modes; normal-sign occupancy kept as a recorded diagnostic; the 3 real cases pass 6/6, sealed-in-panel case documented as correctly refused |
| `18d3cea` | r5: `NEAR_FIELD_ITEMS` → the 7 published near-field items (generation `21a8ec5fc9bd`; the 3 FurnishedRoom ones were unmasked by the 4.3/4.4 fixes), `FLAGGED_ITEMS` 19+7=26, minus-flagged oracle → 0.026; `data/RAF_mappingA/depth_qa_near_field.json` (per-room near-field block + source sha256) commits the publication's own evidence, and three tests bind the registry to it (skip-with-reason if absent, fail on drift) |
| `ab6b1bc` | r5 (sweep smoke fix): `build_per_item_callback` constructs the per-item callback from the SAME resolved metrics config as the headline one (dataset name, window 9600, RT60 policy, flags) and only then switches off the corpus-level aggregation (`dataset_name`→None, `eval_by_scene`→False); `dataset_id=None` had failed validation outright and would otherwise have scored per-item rows on AR's 8000-sample window |
