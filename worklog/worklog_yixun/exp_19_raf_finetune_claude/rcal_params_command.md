...
## Amendment 2 (2026-08-19 ~19:55 EDT) — Leg A restarted: per-seed artifact naming
The first Leg-A loop used a constant `--eval-name exp19_rcal_released`, and `eval_FLAC.py` builds the metrics filename from the eval-name WITHOUT appending the seed (the exp_01 convention embeds the seed in the eval-name string itself). Each seed therefore overwrote the previous seed's `metrics`/`stream` JSONs — seeds 42/43 artifacts were destroyed before detection (caught at seed 44 by the missing `_seed` suffix in the artifact listing). Remedy: loop killed (PIDs verified ours via `/proc/<pid>/cwd` before killing), collided artifacts deleted, ALL FIVE seeds relaunched at ~19:55 with `--eval-name exp19_rcal_released_seed<seed>`. Cost: ~25 min GPU-1 time + a delayed handoff to the peer session (notified). Registered consequence for exp_19 proper: every RAF eval manifest names its cell as `<eval-name>_seed<seed>`; the run driver asserts the artifact filename contains the seed before the run starts.
