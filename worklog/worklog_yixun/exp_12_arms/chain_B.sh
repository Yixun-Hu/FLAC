...
CKPT=$(ls outputs_FLAC/exp12B_ssl_cond/*/*/checkpoints/*step=67500.ckpt 2>/dev/null | head -1)
if [ -z "$CKPT" ]; then
...
K_VALUES=1 bash $REC/eval_arm.sh exp12B_ssl_cond $REC/FLAC_AR_exp12B.json 0 "$CKPT" >> $LOG 2>&1 &
E1=$!
K_VALUES=8 bash $REC/eval_arm.sh exp12B_ssl_cond $REC/FLAC_AR_exp12B.json 1 "$CKPT" >> $LOG 2>&1 &
E8=$!
wait $E1; R1=$?
wait $E8; R8=$?
say "eval finished: K=1 rc=$R1  K=8 rc=$R8"

N=$(find outputs_FLAC/exp12B_ssl_cond -name "*step=67500_metrics*D1*.json" | wc -l)
