...
**Outcome:** registered NULL (0/8 co-occurrence qualifiers) with the mechanism CONFIRMED (band SD ratios 0.570/0.605 vs matched untreated control). The experiment cleanly separates two things the program had conflated: **oscillation width** (decay fixes it) and **metric trade structure** (decay cannot — the tail converges to a different Pareto point: program-best C50, released-level R@1, T60 +0.7 off the anchor).
...
**What it changes:** (1) stop chasing all-cells co-occurrence via schedules at this budget — it's a selection phenomenon over wide orbits; (2) the tail is a *dial*: if a deployment weights C50/R@1, S93750-class checkpoints are obtainable cheaply (5.9 GPU-h) from any anchor; (3) the two-checkpoint paper story (anchor + equivariant Fw) stands unaffected; a "decay-tail S93750" row is an optional third flavor, 5-seed-confirmable on request (~1.5 h).
