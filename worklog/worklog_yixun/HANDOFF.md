...
- **B-F: GO GIVEN (2026-07-16) for the post-extend slot.** Pre-staged kit: `bf_scratch_launch.sh` (GPU-free guard + wandb identity gate + pin gate), `bf_screen.sh` (per-10k-ckpt EMA+online screens), `FLAC_AR_BF_online_eval.json` (use_ema-only flip, diff-verified). Launch sequence when the extend completes: run 90k/100k screens → `LOGGER=none bash worklog/worklog_yixun/exp_07_fa_scratch_claude/bf_scratch_launch.sh` → arm ckpt monitor + screens at 10k cadence.
