...
- **State at 23:02 EDT:** both arms' step-2,500 checkpoints validated (`validate --for-resume` OK; lr 4.99376e-5 == exp_13). Analysis tooling (round D3) Codex-cleared: `python -m src.tools.data_curve.{assemble,import_cells,plot}` on the exp-14 branch. Round T (generator e2e test) landed on `localization-exp` (`4f5a5cd`, `94b5f64`, test-only). **Pre-publication TODO (before the first `model_comparison.md` update ≈ Sep 19):** round U on `localization-exp` — generator `data_curve` contract (exactly seeds 42–46, common ckpt_sha256, protocol), agg errors ⇒ BLOCKED, basename routing, row-local one-angle disclosure (**needs Yixun's D1 answer**); round V on the exp-14 branch — importer attestation manifest + vanilla-row label. See `issue_report.md`.
...
- **Eval-time code drift guard:** the launcher runs `eval_FLAC.py` + `names.py` CLIs from the worktree at eval time; before ≈ Sep 18 12:00 confirm `git diff 6690157 HEAD -- eval_FLAC.py src/tools/data_curve/names.py src/training/run_contract.py` is empty or additive-only (reviewed). Round D3 tooling lives in NEW files.
...
---
## Handoff snapshot 2026-08-29 06:40 EDT (date-verified) (post exp_22 P1 closure)
- **NOTHING IS RUNNING.** Both GPUs idle; no Slurm jobs; no subagent rounds in flight.
- **exp_22 P1 phase CLOSED** (Codex r9z7 APPROVE). Everything committed + pushed on `localization-exp` (HEAD = closure commit). Results: `exp_22_loc_meshgrid_claude/loc_meshgrid_results.md`. To run BF/YAW arms on Yixun's go: reuse the reviewer's P1 launch pattern (scratchpad run_p1_full.sh shape) with `weights/exp20/{BF,YAW}_40k.ckpt` + `--cond-method fa_invariant` FOR BF ONLY if that is its trained protocol — CHECK the ckpt's training protocol first (CLAUDE.md eval-protocol warning); YAW is vanilla-eval. Registered ckpt registry in meshgrid_report.py REGISTERED_CKPT_SHA256 already pins both.
- Probe reruns MUST use emit-then-run launch records from a tracked-clean tree (see loc_meshgrid_command.md).
- Waveform dumps NAS: /media/diskstation/yixunhu/FLAC/checkpoints/exp22_loc_meshgrid/ (use cp, not rsync — CIFS mkstemp fails).
- Session clock drifts: ALWAYS verify `date` before stamping.
