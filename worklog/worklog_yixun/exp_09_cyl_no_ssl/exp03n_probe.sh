#!/usr/bin/env bash
# ============================================================================
# exp03n_probe.sh - exp_03 (max-pool + MLP head) LIFECYCLE PROBE (plan §4.4).
#
# exp03n_launch.sh pins --max-steps 67500 with no override interface, so the probe is a
...
# exp03n_frozen_min_free.txt: that number is committed into the records by hand, pre-launch,
...
# same pin gate (assert_arm_configs_exp03n.py with BOTH pins REQUIRED), the same wandb
...
#
# Scientific shape is the arm's: FLAC_AR_exp03n.json, micro-batch 32/GPU x 2 GPUs x accum 1
# (eff 64), SyncBN, ddp_find_unused_parameters_true, seed 42, num-workers 6.
#
# Usage (from the Slurm wrapper probe_exp03n.sbatch):
#   EXPECT_PACKAGE_SHA=<cyl HEAD> EXPECT_EXP09_SHA=<worktree HEAD> bash exp03n_probe.sh
# ============================================================================
set -uo pipefail
cd /n/fs/gatrdp/codespace/exp03-maxpool-mlp-cond || exit 3   # absolute worktree root for THIS cluster
...
RECORDS="/n/fs/gatrdp/codespace/cylindrical-dinov3/worklog/worklog_yixun_neuronic/exp_03_maxpool_mlp_cond_claude"
LOGGER="${LOGGER:-wandb}"
MB="${MB:-32}"; ACC="${ACC:-1}"
SAVE="${SAVE:-/n/fs/gatrdp/outputs/exp03n_maxpoolmlp_PROBE}"
TS="$(date '+%Y-%m-%d_%H-%M-%S')"
LOG="${EXP03N_LOG_DIR:-$RECORDS}/maxpool_mlp_cond_${TS}_j${SLURM_JOB_ID:-nojob}_probe.log"
...
FROZEN_FILE="${EXPDIR}/exp03n_frozen_min_free.txt"
...
EXPECT_PACKAGE_SHA="${EXPECT_PACKAGE_SHA:-}"; EXPECT_EXP09_SHA="${EXPECT_EXP09_SHA:-}"
[ -n "$EXPECT_PACKAGE_SHA" ] && [ -n "$EXPECT_EXP09_SHA" ] || { echo "REFUSING: both EXPECT_PACKAGE_SHA (cylindrical-dinov3 HEAD) and EXPECT_EXP09_SHA (worktree HEAD) must be set"; exit 3; }

mkdir -p "$(dirname "$LOG")" || exit 3
exec > >(tee -a "$LOG") 2>&1
echo "=== exp03n lifecycle probe (30 -> resume -> 40) - ${TS} - $(git rev-parse --short HEAD 2>/dev/null) - ${MB}x2x${ACC} eff64 seed42 - job ${SLURM_JOB_ID:-nojob} ==="
...
HF_HUB_OFFLINE=1 python "${EXPDIR}/assert_arm_configs_exp03n.py" --expect-package-sha "$EXPECT_PACKAGE_SHA" --expect-exp09-sha "$EXPECT_EXP09_SHA" || { echo "GATE FAILED - abort"; exit 1; }
...
COMMON=( --model-config "${EXPDIR}/FLAC_AR_exp03n.json"
  ...
  --logger "$LOGGER" --name FLAC_exp03n_maxpoolmlp_probe --experiment-name exp03n_maxpoolmlp_probe
  --save-dir "$SAVE" )
...
echo "NEXT STEP (records, by hand + review): write ${RECOMMENDED} into ${FROZEN_FILE} and commit it BEFORE the full launch; exp03n_launch.sh REFUSES while that file is absent."
echo "PROBE PASS: save + resume validated (30 -> 40) with the exp03n max-pool + MLP head"
