...
# --- import-pin sha256 re-verification (replaces the exp-09 pin gate for this non-exp-09 config) ---
(cd "$PDIR/p1_import" && sha256sum -c "$(basename "$PINS")" --quiet) || { echo "IMPORT PIN GATE FAILED — copies do not match p1_import_pins.txt"; exit 1; }
...
exec > >(tee -a "$LOG") 2>&1
echo "=== exp_06-addendum P1 eval — ${TS} — worktree $(git rev-parse --short HEAD) — ${NAME} gpu=${GPU} MIN_FREE_MB=${MIN_FREE_MB} ==="
sha256sum "$CKPT" "$CFG"
