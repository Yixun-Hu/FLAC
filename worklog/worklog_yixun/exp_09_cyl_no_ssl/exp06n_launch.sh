#!/usr/bin/env bash
# EXP06 DIFF 1/19: exp_06 (exp06n) MAX-POOL + BARE-LINEAR conditioning head. This file is exp09_launch.sh with 19 marked non-scientific diffs (absolute cluster roots + PYTHONPATH, log path, frozen-VRAM file, gate script, model config, run names, save-dir, REQUIRED pins, netrc identity gate, banners). EVERY scientific flag below is byte-identical to exp-09's reviewed launcher; the ONLY model delta is cond_pool=max_linear in FLAC_AR_exp06n.json.
# ============================================================================
...
# no extra frame-average passes), PLUS the exp_06 max-pool + bare-Linear conditioning head. See FLAC_AR_exp06n.json and its per-variant config-contract test.   # EXP06 DIFF 14/19: arm description
#
...
# FROZEN threshold MIN_FREE_MB (= measured exp06n L40 peak x 1.15) that exp06n_probe.sh   # EXP06 DIFF 15/19: threshold provenance
# derived, read from the records file exp06n_frozen_min_free.txt: absent file, non-numeric   # EXP06 DIFF 17/19: records file name
...
LOG="${EXP06N_LOG_DIR:-/n/fs/gatrdp/codespace/cylindrical-dinov3/worklog/worklog_yixun_neuronic/exp_06_maxpool_linear_cond_claude}/maxpool_linear_cond_${TS}_j${SLURM_JOB_ID:-nojob}_train.log"   # EXP06 DIFF 4/19: teed log into the exp_06 records folder, job-id stamped
...
FROZEN_FILE="${C1_FROZEN_MIN_FREE_FILE:-${EXPDIR}/exp06n_frozen_min_free.txt}"   # EXP06 DIFF 5/19: exp-09's frozen value is A6000-derived (handoff trap 3); THIS arm's L40 value is derived by exp06n_probe.sh and frozen here. Absent file => REFUSE.
[ -f "$FROZEN_FILE" ] || { echo "REFUSING TO LAUNCH: frozen-records file '${FROZEN_FILE}' is absent - exp06n_probe.sh (plan §4.4) has not FROZEN this arm's free-VRAM threshold yet."; exit 3; }   # EXP06 DIFF 16/19: refusal message names this arm's probe
...
exec > >(tee -a "$LOG") 2>&1
echo "=== exp06n maxpool+Linear DDP+SyncBN - ${TS} - $(git rev-parse --short HEAD 2>/dev/null) - ${MB}x2x${ACC} eff64 seed42 -> 67500 - logger=${LOGGER} - job ${SLURM_JOB_ID:-nojob} ==="   # EXP06 DIFF 6/19: arm banner

# MIN_FREE_MB is bound to the FROZEN threshold above (= this arm's measured L40 peak x 1.15);   # EXP06 DIFF 18/19: threshold provenance
# it is NEVER exp-09's A6000 number nor B-F's inherited 21,900 MiB, and never an arbitrary override.   # EXP06 DIFF 19/19: threshold provenance
...
if [ "$LOGGER" = "wandb" ]; then
  [ -r "$HOME/.netrc" ] || { echo "wandb identity: ~/.netrc is absent/unreadable - run 'wandb login' or use LOGGER=none - abort"; exit 2; }   # EXP06 DIFF 7/19: on this cluster wandb auth lives in ~/.netrc (the bashrc WANDB_API_KEY grep is dropped: it silently no-ops here and would let an unauthenticated run start)
  python - <<'PY'
import sys
try:
    import wandb
...
HF_HUB_OFFLINE=1 CUDA_VISIBLE_DEVICES=0,1 python train.py \
  --model-config "${EXPDIR}/FLAC_AR_exp06n.json" `# EXP06 DIFF 10/19: the exp06n model config = exp-09 + cond_pool in BOTH ViT blocks (the ONLY model delta). The backtick-comment expands to nothing and adds no argument (verified).` \
  ...
  --name FLAC_exp06n_maxpoollinear --experiment-name exp06n_maxpoollinear `# EXP06 DIFF 11/19: arm run names` \
  --save-dir /n/fs/gatrdp/outputs/exp06n_maxpoollinear `# EXP06 DIFF 12/19: save-dir on the cluster scratch (outside both repos, exp_02 convention)` \
  ${RESUME_ARGS[@]+"${RESUME_ARGS[@]}"}
rc=$?
echo "=== exp06n maxpool+Linear exited rc=${rc} at $(date '+%Y-%m-%d %H:%M:%S') ==="   # EXP06 DIFF 13/19: arm banner
exit $rc
