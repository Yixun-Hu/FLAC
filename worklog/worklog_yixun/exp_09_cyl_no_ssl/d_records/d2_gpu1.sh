...
for ROT in 0 90; do
  EVAL_GPU=1 EVAL_SEED=42 ROTATE_DEG=$ROT EVAL_STORE_PREDS=1 \
  EVAL_DATASET_CONFIG=src/configs/dataset_configs/AR/eval/acousticroom_unseeneval.json \
  bash worklog/worklog_yixun/exp_09_cyl_no_ssl/d_eval_driver.sh "$CKPT" "$CFG" "exp09_D2_K8" "$LOGS" \
    || { echo "D2_GPU1 ABORT at rot ${ROT} rc=$?"; exit 1; }
done
echo "D2_GPU1 ALL 2 ROTS DONE rc=0"
