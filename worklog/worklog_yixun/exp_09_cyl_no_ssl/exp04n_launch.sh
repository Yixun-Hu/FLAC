#!/usr/bin/env bash
# EXP04 DIFF 1/19: exp_04 (exp04n) MEAN-POOL + MLP conditioning head. This file is exp09_launch.sh with 19 marked non-scientific diffs (absolute cluster roots + PYTHONPATH, log path, frozen-VRAM file, gate script, model config, run names, save-dir, REQUIRED pins, netrc identity gate, banners). EVERY scientific flag below is byte-identical to exp-09's reviewed launcher; the ONLY model delta is cond_pool=mean_mlp in FLAC_AR_exp04n.json.
# ============================================================================
...
# no extra frame-average passes), PLUS the exp_04 mean-pool + MLP conditioning head. See FLAC_AR_exp04n.json and its per-variant config-contract test.   # EXP04 DIFF 14/19: arm description
#
...
# FROZEN threshold MIN_FREE_MB (= measured exp04n L40 peak x 1.15) that exp04n_probe.sh   # EXP04 DIFF 15/19: threshold provenance
# derived, read from the records file exp04n_frozen_min_free.txt: absent file, non-numeric   # EXP04 DIFF 17/19: records file name
...
# ============================================================================
set -uo pipefail
cd /n/fs/gatrdp/codespace/exp04-meanpool-mlp-cond || exit 3   # EXP04 DIFF 2/19: absolute worktree root for THIS cluster (handoff §3.2 hardcoded-root trap; the launcher is invoked from a Slurm wrapper in the cylindrical repo)
export PYTHONPATH=/n/fs/gatrdp/codespace/cylindrical-dinov3/src   # EXP04 DIFF 3/19: process-local package src (reviewed convention; nothing installed into the conda env)
...
LOG="${EXP04N_LOG_DIR:-/n/fs/gatrdp/codespace/cylindrical-dinov3/worklog/worklog_yixun_neuronic/exp_04_meanpool_mlp_cond_claude}/meanpool_mlp_cond_${TS}_j${SLURM_JOB_ID:-nojob}_train.log"   # EXP04 DIFF 4/19: teed log into the exp_04 records folder, job-id stamped
...
FROZEN_FILE="${C1_FROZEN_MIN_FREE_FILE:-${EXPDIR}/exp04n_frozen_min_free.txt}"   # EXP04 DIFF 5/19: exp-09's frozen value is A6000-derived (handoff trap 3); THIS arm's L40 value is derived by exp04n_probe.sh and frozen here. Absent file => REFUSE.
[ -f "$FROZEN_FILE" ] || { echo "REFUSING TO LAUNCH: frozen-records file '${FROZEN_FILE}' is absent - exp04n_probe.sh (plan §4.4) has not FROZEN this arm's free-VRAM threshold yet."; exit 3; }   # EXP04 DIFF 16/19: refusal message names this arm's probe
...
exec > >(tee -a "$LOG") 2>&1
echo "=== exp04n meanpool+MLP DDP+SyncBN - ${TS} - $(git rev-parse --short HEAD 2>/dev/null) - ${MB}x2x${ACC} eff64 seed42 -> 67500 - logger=${LOGGER} - job ${SLURM_JOB_ID:-nojob} ==="   # EXP04 DIFF 6/19: arm banner

# MIN_FREE_MB is bound to the FROZEN threshold above (= this arm's measured L40 peak x 1.15);   # EXP04 DIFF 18/19: threshold provenance
# it is NEVER exp-09's A6000 number nor B-F's inherited 21,900 MiB, and never an arbitrary override.   # EXP04 DIFF 19/19: threshold provenance
...
if [ "$LOGGER" = "wandb" ]; then
  [ -r "$HOME/.netrc" ] || { echo "wandb identity: ~/.netrc is absent/unreadable - run 'wandb login' or use LOGGER=none - abort"; exit 2; }   # EXP04 DIFF 7/19: on this cluster wandb auth lives in ~/.netrc (the bashrc WANDB_API_KEY grep is dropped: it silently no-ops here and would let an unauthenticated run start)
  python - <<'PY'
import sys
try:
    import wandb
...
EXPECT_PACKAGE_SHA="${EXPECT_PACKAGE_SHA:-}"; EXPECT_EXP04_SHA="${EXPECT_EXP04_SHA:-}"; [ -n "$EXPECT_PACKAGE_SHA" ] && [ -n "$EXPECT_EXP04_SHA" ] || { echo "REFUSING TO LAUNCH: both EXPECT_PACKAGE_SHA (cylindrical-dinov3 HEAD) and EXPECT_EXP04_SHA (worktree HEAD) must be set - no defaults, no fallback set"; exit 3; }   # EXP04 DIFF 8/19: BOTH pins REQUIRED
HF_HUB_OFFLINE=1 python "${EXPDIR}/assert_arm_configs_exp04n.py" --expect-package-sha "$EXPECT_PACKAGE_SHA" --expect-exp04-sha "$EXPECT_EXP04_SHA" || { echo "GATE FAILED - abort"; exit 1; }   # EXP04 DIFF 9/19: exp04n gate (config contract + live arm wiring + bitwise common-init)
...
HF_HUB_OFFLINE=1 CUDA_VISIBLE_DEVICES=0,1 python train.py \
  --model-config "${EXPDIR}/FLAC_AR_exp04n.json" `# EXP04 DIFF 10/19: the exp04n model config = exp-09 + cond_pool/cond_mlp_hidden in BOTH ViT blocks (the ONLY model delta). The backtick-comment expands to nothing and adds no argument (verified).` \
  ...
  --name FLAC_exp04n_meanpoolmlp --experiment-name exp04n_meanpoolmlp `# EXP04 DIFF 11/19: arm run names` \
  --save-dir /n/fs/gatrdp/outputs/exp04n_meanpoolmlp `# EXP04 DIFF 12/19: save-dir on the cluster scratch (outside both repos, exp_02 convention)` \
  ${RESUME_ARGS[@]+"${RESUME_ARGS[@]}"}
rc=$?
echo "=== exp04n meanpool+MLP exited rc=${rc} at $(date '+%Y-%m-%d %H:%M:%S') ==="   # EXP04 DIFF 13/19: arm banner
exit $rc
