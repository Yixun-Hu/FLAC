#!/usr/bin/env python3
"""exp_03 trained-checkpoint equivariance guard (plan §4.5b / §4.6).

The exp-09 guard (``guard_c`` / A2b) reads ``pooler_output`` — the OLD mean pool. This
ADAPTED guard loads a TRAINED exp03n checkpoint and tests the condition the model
ACTUALLY serves, ``MLP(amax(last_hidden_state, dim=1))``, end-to-end through
...
The property is ARCHITECTURAL (azimuth roll permutes the token axis; ``amax`` over that
axis is permutation-invariant; the MLP is pointwise on the pooled vector), so it must hold
at any weights — training can only expose a wiring/gauge regression, never create one.
...
CPU-only and offline by default. Usage:
    ...
    HF_HUB_OFFLINE=1 python worklog/worklog_yixun/exp_09_cyl_no_ssl/guard_exp03n_equivariance.py \\
        /n/fs/gatrdp/outputs/exp03n_maxpoolmlp/.../epoch=..-step=40000.ckpt
...
DEFAULT_CONFIG = os.path.join(HERE, "FLAC_AR_exp03n.json")
...
def check_equivariance(geom, angles=DEFAULT_ANGLES, bound=DEFAULT_BOUND,
    ...
    ``{angle: residual}``; raises RuntimeError if the conditioner is not the max-pool arm,
    if an angle is not a whole token column, or if any residual exceeds ``bound``."""
    if getattr(geom, "dino_pool", None) != "max":
        raise RuntimeError(
            f"guard scope: conditioner dino_pool={getattr(geom, 'dino_pool', None)!r}, expected "
            "'max' — this guard certifies the exp_03 max-pool head, and would otherwise silently "
    ...
    if verbose:
        print("[guard] A2b relative-L2 residual of MLP(amax(tokens)) per angle: "
...
def load_geometry_conditioners(ckpt_path, model_config_path=DEFAULT_CONFIG, device="cpu"):
    """Build the exp03n model from its config, load the checkpoint into it with eval_FLAC's
...
def main(argv=None):
    parser = argparse.ArgumentParser(description="exp03n trained-checkpoint equivariance guard")
    ...
    parser.add_argument("--model-config", default=DEFAULT_CONFIG,
                        help="the exp03n model config the checkpoint was trained with")
    ...
    print(f"GUARD PASS: the served condition MLP(amax(tokens)) is yaw-invariant within "
