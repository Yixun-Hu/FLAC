...
TS=$(date +%Y-%m-%d_%H-%M-%S); PIDS=()
for ARM in cyl van; do
  LOG=$REC/rung6_fit_${ARM}_${TS}.log; echo "=== rung6 $ARM start $(date -Is) FLAC=$(git rev-parse --short HEAD)" > "$LOG"
...
for p in "${PIDS[@]}"; do wait $p; echo "=== rung6 pid $p rc=$? $(date -Is)" >> $REC/rung6_fit_cyl_${TS}.log; done
