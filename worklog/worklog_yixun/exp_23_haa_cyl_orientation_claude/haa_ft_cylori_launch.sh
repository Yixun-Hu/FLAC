...
FREE=$(nvidia-smi --query-gpu=memory.free --format=csv,noheader,nounits -i "$GPU"); echo "GPU ${GPU} free ${FREE} MiB (floor 20000)"
[ "$FREE" -ge 20000 ] || { echo "not enough free VRAM - abort"; exit 2; }
DISK=$(df -Pm . | awk 'NR==2{print $4}'); echo "disk free ${DISK} MiB (floor 80000)"
[ "$DISK" -ge 80000 ] || { echo "not enough disk - abort"; exit 2; }
...
ARGV=("$PY" train.py --dataset-config "$DS" --val-dataset-config "$VDS" --model-config "$CFG"
  ...
  --max-steps "$STEPS" --batch-size 16 --accum-batches 4 --num-workers 8 --seed 42 --num-gpus 1 --precision bf16-mixed
