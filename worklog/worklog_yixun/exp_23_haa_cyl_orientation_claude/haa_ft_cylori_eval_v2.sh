...
GPU="${GPU:-0}"; WHAT="${1:-all}"; ARM="${ARM:-CYLORI}"; ENDPOINTS="${ENDPOINTS:-1000 410}"; TAG="${TAG:-}"
...
if [ "$ARM" = "P1" ]; then CFG=src/configs/model_configs/FLAC/HAA/FLAC_HAA_finetune.json; K8=src/configs/dataset_configs/HAA/eval/haa_test.json; K1=src/configs/dataset_configs/HAA/eval/haa_test_1.json; PROT=(--cond-method vanilla); PYPATH=""
else CFG=$E/FLAC_HAA_finetune_${ARM}.json; K8=$E/haa_test_ori.json; K1=$E/haa_test_1_ori.json; PROT=(--cond-method fa_invariant --frame-avg-angles 0 --frame-avg-max-fwd-samples 64); PYPATH="$CYL_PKG"; fi
TS="$(date '+%Y-%m-%d_%H-%M-%S')"; exec > >(tee -a "$E/haa_ft_${ARM}${TAG}_eval_${TS}.log") 2>&1
echo "=== exp_23 ${ARM}${TAG} eval ${WHAT} endpoints=${ENDPOINTS} | ${TS} | FLAC $(git rev-parse --short HEAD) | pkg $(git -C /home/yixunhu/codespace/cylindrical-dinov3 rev-parse --short HEAD) ==="
sha256sum eval_FLAC.py "$CFG" "$K8" "$K1" "$E/HAA_md_ori.py"
CELLS=()
add() { # step K seed
  local CKS=( outputs_FLAC/exp23_HAA_${ARM}${TAG}/*/*/checkpoints/epoch=*-step=$1.ckpt ); [ -e "${CKS[0]}" ] || { echo "!! no ckpt for step $1"; return; }
  local NAME="exp23_HAA_${ARM}${TAG}_S$1_K$2_s$3"; local DC; [ "$2" = 8 ] && DC=$K8 || DC=$K1
...
run_cell() { IFS='|' read -r CK DC SEED NAME <<< "$1"
  ...
  HF_HUB_OFFLINE=1 PYTHONPATH="$PYPATH" CUDA_VISIBLE_DEVICES="$GPU" $PY eval_FLAC.py --model-config "$CFG" --dataset-config "$DC" --ckpt-path "$CK" \
