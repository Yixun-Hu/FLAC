...
CFG=$E/FLAC_HAA_finetune_CYLORI.json; K8=$E/haa_test_ori.json; K1=$E/haa_test_1_ori.json
TS="$(date '+%Y-%m-%d_%H-%M-%S')"; exec > >(tee -a "$E/haa_ft_cylori_eval_${TS}.log") 2>&1
echo "=== exp_23 CYLORI eval ${WHAT} | ${TS} | FLAC $(git rev-parse --short HEAD) | pkg $(git -C /home/yixunhu/codespace/cylindrical-dinov3 rev-parse --short HEAD) ==="
sha256sum eval_FLAC.py "$CFG" "$K8" "$K1" "$E/HAA_md_ori.py"
CELLS=()
add() { # step K seed
  local CKS=( outputs_FLAC/exp23_HAA_CYLORI/*/*/checkpoints/epoch=*-step=$1.ckpt ); [ -e "${CKS[0]}" ] || { echo "!! no ckpt for step $1"; return; }
  local NAME="exp23_HAA_CYLORI_S$1_K$2_s$3"; local DC; [ "$2" = 8 ] && DC=$K8 || DC=$K1
  if ls "$(dirname "${CKS[0]}")"/*"${NAME}"*.json >/dev/null 2>&1; then echo "skip: $NAME"; return; fi
  CELLS+=("${CKS[0]}|$DC|$3|$NAME")
}
if [ "$WHAT" = all ] || [ "$WHAT" = endpoints ]; then for S in 1000 410; do for K in 8 1; do for s in 42 43 44 45 46; do add $S $K $s; done; done; done; fi
