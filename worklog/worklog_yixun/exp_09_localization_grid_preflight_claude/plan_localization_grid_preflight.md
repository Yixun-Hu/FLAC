...
**2026-08-24 execution amendment (authorized by Query 6, corrected after the real AGREE smoke).** Run the sparse real-RIR control on the union of the two completed non-overlapping 64-query pilots. The candidate bank for a query is every released real RIR sharing its receiver, numerically sorted by source ID and paired with the released metadata coordinate. Replace the FLAC-generated waveform at each candidate with its released real RIR. AGREE's VAE acoustic tower was verified to sample in `eval()` mode, so the observation is independently encoded once and every real candidate is independently encoded eight times with fixed, role-separated per-query seeds; directly dotting the target feature with itself is prohibited. Apply the same nested `K={1,4,8}`, cosine similarity, and stable `tau=0.1` log-mean-exp score as the completed FLAC arms. Report K-specific top-1/error/success plus primary K=8 target score, hardest-negative score and distance, target-minus-hardest-negative margin, visualization softmax mass at `T=0.1`, normalized entropy, and probability mass within 0.5/1.0 m. Produce deterministic score-field examples and aggregate ambiguity plots. Label this a sparse ground-truth-RIR upper bound, not an identity result or the dense-grid geometric oracle. Do not alter or overwrite either FLAC arm's stored artifacts.
...
8. Cache-enabled, no-quality throughput probe projects all fixed `K∈{1,4,8}` prefixes before reading scores; because prefixes are nested, launch cost is governed by K=8. Record the post-probe user cost decision before reading scores.
...
2. **Compute scale:** the review measured 25,312,262 raw candidate-query pairs before mesh/z masking. Query/receiver caching and the geometry-only post-G1 gate remain mandatory. Yixun's later `K∈{1,4,8}` decision supersedes the provisional runtime ladder; no spacing/query/K reduction is automatic.
...
- **B3 accepted, later K-amended by Yixun:** cache query- and receiver-candidate-invariant conditioning, move exact counts/cost approval after G1 and before I1, and adopt context-derived z-band only under its no-new-unwinnable gate. The original `K=4 -> 2 -> 1` ladder is superseded by fixed nested readouts `K∈{1,4,8}`.
