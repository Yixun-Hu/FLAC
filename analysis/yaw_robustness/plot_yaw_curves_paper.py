#!/usr/bin/env python3
"""Plot paper-styled matched-budget yaw robustness curves from exp_10 A6 JSONs."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path(__file__).resolve().parent / "generated_paper",
    )
    parser.add_argument(
        "--mode",
    ...
    for ax, (metric, (ylabel, _)) in zip(axes.flat, metrics.items()):
        for method in METHODS:
            for k, linestyle, fillstyle in ((1, "--", "none"), (8, "-", "full")):
                ...
                label = f"{DISPLAY_NAMES[method]}, K={k}"
                ax.errorbar(
                    angles,
                    means,
                    yerr=stds,
                    ...
                    markerfacecolor=(
                        "white" if fillstyle == "none" else PAPER_COLORS[method]
                    ),
                    markeredgecolor=PAPER_COLORS[method],
...
def write_readme(path: Path, include_c4: bool) -> None:
    c4_text = (
        " `yaw_c4_metrics_paper` is the single 2×3 figure for the measured C4 angles "
        "0°/90°/180°/270°." if include_c4 else ""
    )
    path.write_text(
        "# Matched-budget yaw robustness curves (paper style)\n\n"
        "Comparison: OrbitRIR versus FLAC on the full AR unseen split. Each point "
        "is the mean over evaluation seeds 42–46; error bars show sample standard "
        "deviation (ddof=1). Both K=1 and K=8 are shown. The legacy figures contain "
        "0°, 45°, and 90°; connecting lines do not imply measurements at intermediate "
        "angles.\n\n"
        "`yaw_acoustic_metrics_paper` contains T60, C50, EDT, and FD. "
        "`yaw_retrieval_metrics_paper` contains all six reported retrieval metrics. "
        "Invalid T60 is omitted because it is identically zero. The CSV files contain "
        "the plotted summaries and all source JSON paths."
        + c4_text
        + "\n",
        encoding="utf-8",
    )
...
def main() -> None:
    ...
    if args.mode in ("legacy", "all"):
        ...
        write_csv(args.output_dir / "yaw_metrics_raw_paper.csv", raw_rows)
        write_csv(args.output_dir / "yaw_metrics_summary_paper.csv", summary_rows)
        plot_grid(
            summary_rows,
            ACOUSTIC_METRICS,
            (2, 2),
            args.output_dir / "yaw_acoustic_metrics_paper",
            LEGACY_ANGLES,
        )
        plot_grid(
            summary_rows,
            RETRIEVAL_METRICS,
            (2, 3),
            args.output_dir / "yaw_retrieval_metrics_paper",
            LEGACY_ANGLES,
        )
    if args.mode in ("c4", "all"):
        c4_raw, c4_summary = collect(args.flac_root.resolve(), C4_ANGLES)
        write_csv(args.output_dir / "yaw_c4_metrics_raw_paper.csv", c4_raw)
        write_csv(args.output_dir / "yaw_c4_metrics_summary_paper.csv", c4_summary)
        plot_grid(
            c4_summary,
            C4_FIGURE_METRICS,
            (2, 3),
            args.output_dir / "yaw_c4_metrics_paper",
            C4_ANGLES,
        )
    write_readme(args.output_dir / "README_paper.md", args.mode in ("c4", "all"))
