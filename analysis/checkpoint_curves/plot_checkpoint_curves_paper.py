#!/usr/bin/env python3
"""Plot paper-styled yaw-zero checkpoint performance curves."""
...
OUT = HERE / "generated_paper"
STEPS = tuple(range(2500, 40001, 2500))
DISPLAY_NAMES = {
    "Vanilla FLAC": "FLAC",
    "Per-angle FA": "OrbitRIR",
}
PAPER_COLORS = {
    "Vanilla FLAC": "#1F77B4",
    "Per-angle FA": "#FF7F0E",
...
def main() -> None:
    ...
    csv_path = OUT / "checkpoint_performance_seed42_paper.csv"
        ...
        for method in ("Vanilla FLAC", "Per-angle FA"):
            for k in (1, 8):
                ...
                ax.plot(
                    ...
                    color=PAPER_COLORS[method], linestyle=line, marker=marker,
                    markerfacecolor=face or PAPER_COLORS[method],
                    markeredgecolor=PAPER_COLORS[method], linewidth=1.7,
                    markersize=4.2, label=f"{DISPLAY_NAMES[method]}, K={k}",
                )
    ...
    for suffix in ("png", "pdf"):
        fig.savefig(
            OUT / f"checkpoint_performance_seed42_paper.{suffix}",
            dpi=300,
            bbox_inches="tight",
        )
    plt.close(fig)
    print(f"Wrote {csv_path} and checkpoint_performance_seed42_paper.{{png,pdf}}")
