...
- ``probe_rir_encoder`` -- end-to-end (needs the real ckpt+dataset; the B-1
  pilot covers it, so only its pure parts are unit-tested).

Run directly:
    python tools/bn_drift_probe.py --ckpt-path weights/FLAC/FLAC_EMA.ckpt \\
        --dataset-config <dataset.json> --n-batches 200 --batch-size 16 --out r.json
"""
import os
import sys
...
class BNInputRecorder:
    ...
    combine in float64 so many batches stay accurate. The hooks only *read*
    inputs, so a probe pass in eval mode leaves every BN running buffer
    bit-identical (BatchNorm does not update its stats in eval mode).
    """
    ...
    def __init__(self, module):
        ...
        for name, m in module.named_modules():
            if isinstance(m, self._BN_TYPES):
                ...
                self._count[name] = 0
                self._mean[name] = torch.zeros(m.num_features, dtype=torch.float64)
                self._m2[name] = torch.zeros(m.num_features, dtype=torch.float64)
...
def probe_rir_encoder(ckpt_path, dataset_config_path, n_batches, batch_size, device,
    ...
    recorder = BNInputRecorder(model.conditioner.conditioners["context_audio"].net)
