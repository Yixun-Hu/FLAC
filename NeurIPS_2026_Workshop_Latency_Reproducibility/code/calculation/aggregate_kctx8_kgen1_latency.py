...
def render_markdown(payload: dict) -> str:
    single_repeat = int(payload["repeat_count"]) == 1
    lines = [
        (
            "# Localization inference latency: repeat 1 result"
            if single_repeat
            else "# Localization inference latency: final repeat aggregate"
        ),
        "",
        (
            f"Scope: {payload['scope']['room_count']} rooms / "
            f"{payload['scope']['query_count']} frozen queries / "
            f"{payload['repeat_count']} learned-model timing repeat(s). Every method uses "
        ...
        (
            "For each method and query, the reported latency is the repeat-1 value; "
            "table statistics are then computed across frozen queries."
            if single_repeat
            else "For each method and query, the reported latency is the median across "
...
def main() -> None:
    ...
    payload = {
        ...
        "aggregation_protocol": (
            "single repeat, then query micro"
            if len(repeats) == 1
            else "per-query median over repeats, then query micro"
        ),
