...
from .losses import MultiLoss, AuralossLoss, ValueLoss, L1Loss, LossWithTarget, MSELoss
...
class AutoencoderTrainingWrapper(pl.LightningModule):
    def __init__(
        ...
        # Discriminator
        if self.use_disc:
            if loss_config['discriminator']['type'] == 'encodec':
        ...
        # evaluation losses & metrics (for validation during training)
        self.eval_losses = torch.nn.ModuleDict()
        if eval_loss_config is not None:
            if "stft"in eval_loss_config:
                self.eval_losses["stft"] = auraloss.STFTLoss(sample_rate=sample_rate, **eval_loss_config["stft"])
            if "metrics" in eval_loss_config:
                from ..metrics.metric_callback import AcousticMetricsCallback
                self.metric_callback = AcousticMetricsCallback(
                    sample_rate=sample_rate,
                    audio_channels=self.autoencoder.out_channels,
                    dataset_name=eval_loss_config["metrics"].get("dataset_name", "AcousticRooms"),
                    eval_T60=eval_loss_config["metrics"].get("eval_T60", False),
                    eval_C50=eval_loss_config["metrics"].get("eval_C50", False),
                    eval_EDT=eval_loss_config["metrics"].get("eval_EDT", False),
                    eval_l1_distance=eval_loss_config["metrics"].get("eval_l1_distance", False),
                )
        ...
        with torch.no_grad():
           ...
           # Run evaluation metrics.
            ...
            if self.metric_callback is not None:
                self.metric_callback.update_metrics("val", decoded, reals, )
    ...
    def on_validation_epoch_end(self):
        ...
        if self.metric_callback is not None:
            metrics = self.metric_callback.compute_metrics("val")
            for key, value in metrics.items():
                if isinstance(value, torch.Tensor):
                    value = value.item()
                log_metric(self.logger, f"val/{key}", value)
    ...
    def test_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            for eval_key, eval_fn in self.eval_losses.items():
                ...
                test_loss_dict[eval_key] = loss_value

            if self.metric_callback is not None:
                scene = [md[i]['scene'] for i in range(len(md))]
                self.metric_callback.update_metrics("test", decoded, reals, scene=scene)

        self.test_step_outputs.append(test_loss_dict)
        return test_loss_dict
    
    def on_test_epoch_end(self):
        sum_loss_dict = {}
        for loss_dict in self.test_step_outputs:
            for key, value in loss_dict.items():
                if key not in sum_loss_dict:
                    sum_loss_dict[key] = value
                else:
        ...
        for key, value in sum_loss_dict.items():
            test_loss = value / len(self.test_step_outputs)
            test_loss = self.all_gather(test_loss).mean().item()
            # log_metric(self.logger, f"test/{key}", test_loss)

        if self.metric_callback is not None:
            metrics = self.metric_callback.compute_metrics("test")
            for key, value in metrics.items():
                if isinstance(value, torch.Tensor):
                    value = value.item()
                # log_metric(self.logger, f"test/{key}", value)
...
def create_loss_modules_from_bottleneck(bottleneck, loss_config):
    ...
    if isinstance(bottleneck, VAEBottleneck):
        ...
        raise NotImplementedError(f"Loss modules for bottleneck type {type(bottleneck)} not implemented")
