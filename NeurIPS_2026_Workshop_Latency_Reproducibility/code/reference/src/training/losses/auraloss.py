...
class EnergyDecayLoss(torch.nn.Module):
    def __init__(self, name: str = 'energy_decay_loss', distance="L1", reduction="mean", normalize=True):
        super(EnergyDecayLoss, self).__init__()
    ...
    def forward(self, x_mag, y_mag):
        ...
        if len(x_mag.shape) > 3: 
            raise NotImplementedError('STFT with channels higher than 1 are not supported yet.')
        
        x_mag = torch.sum(x_mag, dim=-2) # sum on frequency axis
        y_mag = torch.sum(y_mag, dim=-2)

        energy_x = x_mag ** 2 # power 
        energy_y = y_mag ** 2

        decay_x = torch.flip(torch.cumsum(torch.flip(energy_x, [-1]), dim=-1),[-1]) # cumulative sum (decay of energy)
        decay_y = torch.flip(torch.cumsum(torch.flip(energy_y, [-1]), dim=-1), [-1])

        db_decay_curve_x = 10 * torch.log10(decay_x + 1e-13) 
        db_decay_curve_y = 10 * torch.log10(decay_y + 1e-13)

        if self.normalize: # Few shot RIR uses this
            valid_loss_idxs = ((decay_y != 0.).type(decay_y.dtype))[..., 1:] # remove 0 energy values in GT

            # Normalize to initial value to get relative decay
            norm_db_x = db_decay_curve_x - db_decay_curve_x[..., :1]
            norm_db_y = db_decay_curve_y - db_decay_curve_y[..., :1]

            # Drop first time index to match original logic
            norm_db_x = norm_db_x[..., 1:] 
            norm_db_y = norm_db_y[..., 1:] * valid_loss_idxs

            loss = self.distance(norm_db_x * valid_loss_idxs, norm_db_y * valid_loss_idxs)
        else:
            # Directly compute L1 loss on the decay curves
            loss = self.distance(db_decay_curve_x, db_decay_curve_y)
...
class SpectralConvergenceLoss(torch.nn.Module):
    ...
    def forward(self, x_mag, y_mag):
        sc = torch.norm(y_mag - x_mag, p="fro", dim=[-1, -2]) / torch.norm(y_mag, p="fro", dim=[-1, -2])
...
class STFTLoss(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.spectralconv = SpectralConvergenceLoss(reduction=reduction if not self.retain_batch_dim else "none")
        ...
        if self.perceptual_weighting: # Not optimal for RIR
    ...
    def forward(self, input: torch.Tensor, target: torch.Tensor):
        ...
        ed_mag_loss = self.ed_loss(x_mag, y_mag) if self.w_ed else 0.0
        ...
        if self.output == "loss":
            ...
            return loss, sc_mag_loss, log_mag_loss, lin_mag_loss, phs_loss, ed_mag_loss
...
class MultiResolutionSTFTLoss(torch.nn.Module):
    ...
    def forward(self, x, y):
        mrstft_loss = 0.0
        sc_mag_loss, log_mag_loss, lin_mag_loss, phs_loss, ed_mag_loss = [], [], [], [], []
        ...
        if f.output == "loss":
            ...
            return mrstft_loss, sc_mag_loss, log_mag_loss, lin_mag_loss, phs_loss, ed_mag_loss
