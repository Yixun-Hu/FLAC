...
def execute_baseline_query(
    *,
    ...
    """Execute one query for either direct-waveform or FEM-Sabine synthesis."""
    ...
    if method == "few_shot_rir_waveform":
    ...
    if measure_core_forward:
        core_total = float(core_timing["candidate_conditioning_and_generation"])
        ...
        result["latency_seconds"] = {
            "candidate_conditioning_and_generation": core_total,
...
def run_baseline_localization(
    *,
    ...
    if method not in ("few_shot_rir_waveform", "fem_sabine"):
        raise ValueError("unsupported baseline method")
    ...
    if measure_core_forward and (
        method != "few_shot_rir_waveform" or context_counts != (8,)
    ):
    ...
    if method == "few_shot_rir_waveform":
        if (
            ...
            raise ValueError(
                "Few-ShotRIR-Waveform requires config, checkpoint, and AGREE paths"
            )
        method_identity = {
