...
class CylindricalPatchEmbeddingTest(unittest.TestCase):
    ...
    def test_cnn_stem_contract_and_token_shape(self):
        ...
        for stage, contract in zip(stem.stages, expected):
            ...
            self.assertEqual(stage.conv.padding, (pad_h, pad_w))
            self.assertEqual(stage.conv.padding_mode, "zeros")
        ...
        with torch.no_grad():
            tokens = stem(torch.randn(1, 3, 256, 512))
        self.assertEqual(tokens.shape, (1, 256, 512))

    def test_cnn_stem_patchifies_before_convolution(self):
        stem = CylindricalCNNPatchEmbedding().eval()
        first_stage_input = []

        def capture_input(module, args):
            first_stage_input.append(args[0].shape)

        handle = stem.stages[0].register_forward_pre_hook(capture_input)
        try:
            with torch.no_grad():
                tokens = stem(torch.randn(2, 3, 32, 64))
        finally:
            handle.remove()

        self.assertEqual(first_stage_input, [torch.Size([8, 3, 16, 32])])
        self.assertEqual(tokens.shape, (2, 4, 512))

    def test_cnn_stem_does_not_mix_neighboring_patches(self):
        torch.manual_seed(42)
        stem = CylindricalCNNPatchEmbedding().eval()
        baseline = torch.zeros(1, 3, 32, 64)
        changed = baseline.clone()
        changed[:, :, 16:32, 0:32] = torch.randn(1, 3, 16, 32)

        with torch.no_grad():
            baseline_tokens = stem(baseline)
            changed_tokens = stem(changed)

        token_delta = (changed_tokens - baseline_tokens).abs().amax(dim=-1).squeeze(0)
        # Height-major token order: (h=1, w=0) is token index 2 in a 2x2 grid.
        self.assertGreater(token_delta[2].item(), 1e-6)
