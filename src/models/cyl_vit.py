...
RIR (see ``src.data.yaw_rotation``). :class:`CylindricalViT` is equivariant to
patch-level azimuth rolls by construction (``C16`` at 512/32 = 16 patches).
...
(b) Roll-equivariant body. The selected patch embedding maps the panorama to a ``16x16``
    ...
    ``16x32`` linear projection or a hierarchical CNN with total stride ``(16, 32)``.
...
(c) Readout. The Yaw-equi-DiT caller vertically pools each azimuth column and
    retains all 16 columns. The resulting token sequence circularly shifts under
    yaw and is paired with destination-column phases for relative-phase attention.

Sub-patch rolls (``k`` not a multiple of 32) are not exact. The C16 contract is
covered by the yaw-phase integration tests.
"""
import math
...
class CylindricalConvStage(nn.Module):
    """One strided CNN-stem stage with zero-height and circular-width padding."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Downsample while preserving exact cyclic-shift equivariance in azimuth."""
        x = F.pad(
            x,
            (self.width_padding, self.width_padding, 0, 0),
            mode="circular",
        )
        x = F.pad(
            x,
            (0, 0, self.height_padding, self.height_padding),
            mode="constant",
            value=0.0,
        )
...
class CylindricalCNNPatchEmbedding(nn.Module):
    """Hierarchical parameter-matched CNN patch embedding for the C16 experiment."""
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Return the ``[B, H_tok*W_tok, 512]`` CNN token sequence."""
...
class CylindricalViT(nn.Module):
    ...
    def __init__(
        ...
        patch_embed_type : {"linear", "cnn"}, optional
            ...
            projection; ``"cnn"`` selects the four-stage C16-equivariant CNN stem.
        """
        super().__init__()
        if in_channels != 3:
        ...
        token_cols = torch.arange(self.w_tok, dtype=torch.float32)
        azimuth_phases = (
            (token_cols + 0.5) * (2.0 * math.pi / self.w_tok) - math.pi
        )
        self.register_buffer(
            "azimuth_phases", azimuth_phases, persistent=False
        )
