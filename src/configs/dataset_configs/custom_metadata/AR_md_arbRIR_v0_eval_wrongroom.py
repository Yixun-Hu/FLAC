...
Context-ablation control (wrongroom) for the advisor's "context RIR = material
proxy" hypothesis. Cloned from AR_md_arbRIR_v0_eval_A.py with the pose logic
left BYTE-IDENTICAL; only md['context_audio'] is overridden (a different-room RIR). Scheme-A
...
_SCHEME = "wrongroom"  # Scheme-A poses; context audio from a different room
