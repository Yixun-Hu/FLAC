"""
SUPERSET eval metadata module — Scheme B (frozen-manifest reader).

Plan: plan/eval_arbRIR_v0_vs_baseline_K1_K8.md

Identical to AR_md_arbRIR_v0_eval_A.py in EVERY respect (same frozen manifest,
same picks, same query side, same V3 keys) EXCEPT the single baseline key:

  Scheme B: md['context_poses'] = pair_local = s_i - r_i

i.e. the *literal* AR_md.py formula (context source position in its own
context-receiver frame). Under arbitrary r_i != r_q this differs from Scheme A
(s_i - r_q) and injects the additional moved-origin distribution gap; running
both A and B brackets the baseline's true behaviour. Reads the SAME manifest as
Scheme A, so the picked context is byte-identical — only this assignment
differs. Serves baseline-B runs only (the ablation ignores context_poses).
"""
import os
import json
import numpy as np
...
# ---- Scheme switch (the ONLY semantic difference vs. _eval_A.py) ----
_SCHEME = "B"  # context_poses = pair_local (s_i - r_i)
...
def get_custom_metadata(info, audio):
    ...
    if acoustic_context_config.get('load', False):
        ...
        # Baseline cross-attention contract — SCHEME B: s_i - r_i.
        md['context_poses'] = pair_local             # [N, 3]
